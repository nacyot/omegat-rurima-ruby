...
: Hash#compare_by_identity?: Hash#identical
: Hash#identical?
...
: ((<Dir/Dir.[]>))   [compat]
  ...
  Dir.glob に配列を渡して複数のパターンを指定できるようになりました。  また、Dir[] は、複数の引数を渡すことで複数のパターンを指定できるよう
...
: proc [ruby][parser][experimental]
  ...
  (({(expr)(args...)})) で (({expr.call(args...)})) が呼ばれるようになりました。  この機能は実験的なものです。
...
: Array#[]= [change]
  ...
  a[n,m]=nil は要素の削除ではなくなり、要素の並びを nil に入れ換えるようになりました。  ((<zw-kdoo(2004-10-24)|URL:http://yowaken.dip.jp/tdiary/20041024.html#c02>))
...
: Array#pack [ruby] [change]
  ...
  RangeError を投げるようになりました。  ((<ruby-dev:23281>))
...
: ((<IO/IO.popen>))             [compat]
  ...
  た場合にシェルを経由せずに子プロセスを実行できるようになりました。  ((<ruby-dev:22877>))
