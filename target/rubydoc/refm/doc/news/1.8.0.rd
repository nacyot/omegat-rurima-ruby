...
: ((<組み込み変数/$stdin>))  [change]
  ...
  $defout や $deferr に代入を行うと警告がでます。  (注：1.6 に $deferr はありません)
  ((<ruby-dev:20961>))

  $stdin にオブジェクトを代入すると標準入力からの入力メソッド(gets 等)
  はそのオブジェクトにメソッドを投げます。  (リダイレクトしなくなった点を除けば、1.6 とそれほど違いはないようです)

: ((<組み込み変数/$VERBOSE>))

  三段階のレベルを持つようになりました。  * nil: 警告を出力しない   (-W0 新しい警告レベル)
...
: ((<リテラル/数値リテラル>)) [compat]
  ...
  10進((*整数*))リテラルの prefix として 0d が追加されました。  8進リテラルの prefix として 0 以外に 0o が追加されました。
...
: ((<Proc#to_proc|Proc/to_proc>)) [new]
  ...
  た。以前は、& 修飾できるのは Proc, Method オブジェクト限定でした。  これに伴い Proc#to_proc が追加されました。

: 終了ステータス [compat]

  raise SystemExit したときに終了ステータス 1 で終了するようになりました。  ((<ruby-dev:16776>))
...
: ((<"rescue/ensure on begin .. end while"|制御構造/while 修飾子>)) [compat]
  ...
  るまい(C の do ... while 構文と同じ)をしていませんでした。  ((<ruby-list:34618>))
...
: break and next        [compat]
  ...
  break [n] はイテレータを終了させ、n がそのイテレータの戻り値になります。  next [n] はブロックを抜け、n が yield の戻り値になります。
...
: rescue 節の例外クラスと発生した例外オブジェクトの比較 [ruby] [change]
  ...
  になったのでこの変更の効果はなくなってますが、変更自体は残ってます。  (ユーザで例外クラスを定義するのに使えるかもしれません)
  ((<ruby-dev:19589>))
...
: while, until, class, module, def の値         [ruby] [change]
  ...
  합니다.while/until は、通常 nil を返しますが、break の引数により任意
  の値を返すことができます。
...
: 配列展開 [change]
  ...
  以下の挙動を修正しました。  現在は、1要素の配列も正常に展開されます。
...
: ((<Interrupt>))                       [change]
  Interrupt は、((<SignalException>))のサブクラスになりました。  (1.6以前はExceptionのサブクラス)
...
: ((<Module#autoload?|Module/autoload>))  [new]
  ...
  ネストしたクラス/モジュールに対する autoload 指定が可能になりました。  ((<ruby-dev:16159>)), ((<ruby-dev:16165>)) ((<ruby-dev:18103>)),
  ((<ruby-dev:19686>))

: ((<Module#const_missing|Module/const_missing>))  [new]

   定義されていない定数を使用したときに const_missing という名のメソッドを
   呼ぶようになりました。デフォルトでは NameError 例外を発生させます。   ((<ruby-core:00441>))
...
: ((<String#rstrip!|String/rstrip!>))     [new]
  ...
  追加。左端あるいは右端の空白類を取り除きます。  rstrip は、右側の "\0" も取り除きます。
...
: ((<組み込み関数/exec>)) [compat]
  ...
  同様に、引数として配列が与えられた場合はシェルを経由しないようになりました。  以前は常にシェルを経由していました(win32系portは2003-01-04に変更)。  ((<ruby-dev:19107>))
...
: ((<Array#[]=|Array/[]=>)) [compat]
  ...
  す、例外 ((<TypeError>)) が発生するようになりました。  ((<ruby-list:37217>))
...
: ((<String#unpack|String/unpack>))   [new]
  ...
  64 bit 整数のテンプレート文字 Q/q が追加されました(Quad の意)。  Q は unsigned、q は、signed です。
...
: ((<ENV>)) [change]
  ...
  ENV が生成する文字列はすべて ((<Object/freeze>)) されるようになりました。  ((<ruby-talk:72732>))
...
: ((<Hash/Hash.new>))   [compat]
  ...
  その都度ブロックを実行し、その結果を返します。  ブロックにはハッシュ自身と、ハッシュを参照したときのキーが渡されます
...
: ((<Object#protected_methods|Object/protected_methods>)) [compat]
  ...
  * 将来これらのメソッドの引数のデフォルト値は true になる予定だが、
    ...
    ルト値が false(過去との互換のため。なお、省略したままだと警告が出る)。    デフォルトに頼らないようにするべき、1.6 で使用していたスクリプトを
...
#   実行されます(したがって、$~ は設定されません)。
  ...
  外を設定しても例外 ((<TypeError>)) は発生しないようになりました。  ((<ruby-talk:77381>))
...
: ((<String#[]=|String/[]=>))   [change]
  ...
  第一引数が正規表現を渡す形式で、オプションの第二引数 idx が追加されました。  str[/re/, 0] は、str[/re/] と同じです。
...
: ((<ThreadGroup#list|ThreadGroup/list>)) [compat]
  終了中(aborting)のスレッドもリストに含まれるようになりました。  ((<rubyist:1282>))
...
: ((<Integer#to_s|Integer/to_s>)) [compat]
  ...
  引数に基数(2,8,10,16)を指定できるようになりました。  (2002-01-26: 引数が 0 のときは prefix で基数を判定する)
...
: ((<Comparable>)) [change]
  ...
  (other <=> string)})) の結果を返します。그렇지 않으면 nil을 리턴합니다.
  ((<ruby-dev:19625>))
...
: ((<Dir/Dir.glob>))        [change]
  ...
  できるようになりました。  また、空白類に特殊な意味はなくなりました('\0'の効果は残っています)。
...
: ((<Module#include|Module/include>)) [change], [experimental]
  ...
  なく、wrapper モジュールになりました。  つまり、load(file, true) で、file をロードしたときに file 中で
...
: ((<Object#extend|Object/extend>))  [change]
  ...
  複数のモジュールを渡したときにインクルードされる順序が変更されました。  ((<ruby-dev:16035>))
...
: ((<Process/Process.times>))           [change]
  ((<Time/Time.times>)) から移動しました。  (Time.times も残っていますが、warningが出ます)
...
: [parser]
  ...
  スコープ演算子 `::' を伴う定数代入を許すようになりました。        p Object::Foo = 1
...
: rescue修飾式 [parser] [change]
  ...
  rescue 修飾式の優先度が変わりました。これは、実験的なもののようです。  (1.8 リリースで残っているので正式採用のようです)
...
: メソッド呼び出し      [parser] [change]
  ...
  式の括弧と解釈するようになりました。  (ただし、p (1, 2) とすると空白があっても引数を括る括弧になる。  つまり、(おそらく)意図通りに動作する)
...
: ((<正規表現>)) [regexp]
  ...
  が改行の前だけでなく、常に文字列の末尾にもマッチするようになりました。  これは、perl や python と同じ挙動です。((<ruby-dev:20187>))
...
: ((<Marshal>))         [marshal] [change]
  無名モジュールを include したオブジェクトがダンプできなくなりました。  ((<ruby-dev:18186>))
...
: ((<Marshal>))         [marshal] [change]
  ...
  トバージョンが 4.6 から 4.7 に上がっています。  (この後、strtod(3) の組み込みにより、読み込み時もシステムの strtod(3)
  に依存しなくなっています)
...
* DOSISHなプラットフォームでのドライブレター対応が強化されました。* ((<Process/Process.pid>)) (win)
  ...
  ライブラリの置き場所のパス要素)が "i386-msvcrt" に変更されました。  ((<ruby-dev:17144>)), ((<ruby-dev:18047>))
...
: ((<Process/Process.kill>)) [compat]
  ...
  ChangeLog によると win32 でも、シグナル 0 がサポートされたそうです。  多くの Unix で、シグナル 0 の送信は、プロセスの存在チェックに使用で
...
: ENV["PATH"]    [ruby], [bug?]
  ...
  ク時などに判断)の大文字と小文字を区別しないようになりました。  ((<ruby-dev:20374>))
...
: ((<File/File.join>)) [CHANGE]
  ...
  DOSISHなプラットフォームでのドライブレター対応が強化されました。  ((<DOSISH 対応>))を参照。

: ((<Process/Process.pid>)) (win) [change]

  mswin32 版と mingw32 版で、ruby 内部はプロセスIDを常に正の値に変換して
  扱うようになりました。  NT系のOSでは従来と違いはありませんが、Win9x系のOSでは、OSの保持する
...
: ((<String#~|String/~>))  [obsolete]
  ...
  =~ regexp あるいは regexp =~ string を使うことが推奨されます。  (1.8.1 以降使用できなくなるかもしれません) ((<ruby-list:37662>))

  String#=~ の変更と同様にString#~ で警告が出るようになりました。  (1.8.1 以降使用できなくなるかもしれません)
...
: ((<Range#length|Range/length>)) [obsolete]
  ...
  このメソッドはなくなりました。  ((<ruby-talk:64479>)), ((<ruby-talk:72133>))
...
: ((<tmpdir>)) [new]
  ...
  テンポラリディレクトリを取得するためのライブラリが追加されました。  Windows などで利用されるライブラリは、/tmp をハードコードせずに(ま
  た 環境変数 TEMP などを参照せずに)このライブラリを使用するべきです。  (tempfile などが tmpdir を使用するようになりました。  ((*tmp*))dir と ((*temp*))file というのがまたなんとも・・・)
...
: ((<yaml|YAML>)) [new]
  ...
  追加。YAML は、YAML Ain't Markup Language だそうです。  ((<URL:http://yaml4r.sourceforge.net/>))
  ((<URL:http://yaml.org/>))
...
: 添付ライブラリ [lib] [new]
  ...
  以下のライブラリが新たに追加されました。  ((<iconv>)), ((<tsort>)), ((<StringIO>)), ((<strscan>)),
  ((<fileutils>)), racc/*
...
: rb_undef_alloc_func() [api] [new]
  ...
  追加。((<Class/allocate>)) メソッドの定義に使用します。  ((<ruby-dev:19116>))
...
: STR2CSTR() [api] [new]
    ...
    こからも参照されないため、型変換結果が GC される可能性があります。    ((<ruby-dev:12731>))
    ...
    は、引数が to_str による暗黙の型変換を期待する場合に使用します。    引数が文字列なら何もしません。    文字列を受け取るメソッドの最初の方で読んでおくと便利です。
...
: ((<組み込み関数/trace_var>))  [bug]
  ...
  ベル4で評価するようになっていました。  ((<ruby-list:32215>))

: ((<Array#collect|Array/collect>))   [bug]
: ((<Array#map|Array/map>))       [bug]

  Array#collect がブロックを伴わない場合に self.dup を返していました。  そのため、Array 以外を返すことがありました((<ruby-list:30480>))。
...
: ((<File/File.open>))       [bug]
  ...
  しました。  ((<ruby-bugs-ja:PR#54>))

: ((<IO>)) (win32) [bug]

  mswin32・mingw32で、更新モード(w+,r+)でオープンされたファイルに対する
  読み書きの切り替えがうまくいっていなかった問題が修正されました。  bccwin32にも同様の問題がありますが、こちらは未修正です。  ((<ruby-dev:19299>))

: ((<IO#putc|IO/putc>)) [bug]

  出力メソッドのうち putc だけが write メソッドを使用していませんでした。  ((<ruby-dev:18038>))

: IO#read, gets ..., etc. [bug]

  File::NONBLOCK を指定した IO の読み込みで EWOULDBLOCK が発生すると、
  途中まで読んだデータが失われることがありました。  ((<ruby-dev:17855>))
...
: ((<String#upto|String/upto>)) [bug]
  ...
  String#upto の範囲チェックが辞書順だったバグが修正されました。  現在は、((<String/succ>)) の動作と一致します。
...
: ((<セキュリティモデル>)), ((<組み込み変数/$SAFE>)) [bug], [change]
  ...
  ((<終了処理>))直前に $SAFE が 0 になるように変更されました。  ((<ruby-core:01119>))
...
: ((<"||="|演算子式>)) [bug]
  ...
  が出ていました。また、クラス変数はエラーになっていました。  ((<ruby-dev:18278>))
