...
bundled libraryである(rubyの一部ではない)soap4rの変更点については、以下を参考にしてください。soap4r-1.5.3がruby-1.8.2、soap4r-1.5.5がruby-1.8.3にbundleされています。 * ((<URL:http://dev.ctor.org/soap4r/wiki/Changes-154>))
...
: FileUtils.cp_r  [lib] [compat]
  ...
  rm_f と rm_rf が :secure オプションを受け付けるようになりました。  cd が :noop オプションを受け付けなくなりました。  cp_r が :dereference_root オプションを受け付けるようになりました。
...
: net/http [lib] [new]
  ...
  WebDAV のメソッドをサポートするようになりました。  PROPPATCH, LOCK, UNLOCK, OPTIONS, PROPFIND, DELETE, MOVE, COPY, MKCOL。  
...
: $SAFE [ruby] [compat]
  ...
  メソッドが定義された時の $SAFE レベルを記憶するようになりました。  メソッドは定義された時の $SAFE レベルで実行されます。  $SAFE レベル 3 以上の環境下において定義されたメソッドを呼び出すことは、$SAFE レベル が 0 のとき
  禁止されるようになりました。
...
: Delegator [lib] [compat]
  ...
  Delegator オブジェクトが生成された後に定義されたメソッドに関しても、適切に委譲するようになりました。  ((<ruby-talk:146776>)) ((<ruby-talk:146894>))
    $ cat test_dlg.rb
    foo = Object.new
...
: Module#class_variable_set [ruby] [new]
  ...
  追加されました。((<ruby-talk:144741>))
    ...
    def Fred.foo
      @@foo = 101      #=> @@foo は Fred クラスのクラス変数ではない。    end
    
    def Fred.foo_foo   
      class_variable_set(:@@foo, 101)  # self が Fred クラス自身であることに注意。クラス変数 @@foo に値をセットする。    end
    
    Fred.foo           # メソッドを呼んでも、Fred クラスのクラス変数 @@foo は変わらない。    p Fred.new.foo     #=> 99 
...
: WEBrick::CGI::Socket#request_line [lib] [compat]
  ...
  request_line メソッドは REQUEST_URI ヘッダがあればそちらを優先して使うようになりました。  ((<ruby-dev:26235>))
...
: local variable and method [ruby][bug]
  ...
  て影響されるバグが修正されました。  ((<ruby-dev:25737>))
...
: ((<クラス／メソッドの定義/defined?>)) [ruby][bug]
  ...
  "assignment" を返すようになりました。  ((<"[yarv-dev:418]"|URL:http://www.atdot.net/mla/yarv-dev/418>))
...
: IO#readpartial [new]
  ...
  "nonblocking IO#read is obsolete" という警告が出るようになりました。  そして ruby 1.8 の nonblocking IO#read の移行先として
  ruby 1.8.3 に IO#((<IO/readpartial>)) が追加されました。  ((<ruby-dev:25430>)) ((<ruby-dev:25443>))
