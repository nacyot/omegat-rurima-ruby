...
: 2002-08-19 Thread (win)
  ...
  クを含む）を同時に使うと落ちてしまう不具合が修正されたのだそうです。  ((<ruby-win32:273>))
...
: 2002-05-02 Regexp.quote
  ...
  正規表現を //x に正しく埋め込めるようにするためです。  ((<ruby-bugs-ja:PR#231>))
...
: 2002-04-15: pack('U')
  ...
  pack('U') を unpack('U') すると元に戻らないバグが修正されました。  (unpack は、バイト単位でなく文字単位の処理になりました)
  ((<ruby-bugs-ja:PR#220>))
...
: 2002-03-16 $~
  ...
  $~ の状態をクリアしていませんでした。  ((<ruby-bugs-ja:PR#208>))
...
: 2002-03-10 メソッドの戻り値
  ...
  * ((<Process/Process.setpgrp>)) が返す値が不定だった。  * ((<String/ljust>)), ((<String/rjust>)), ((<String/center>)) の結果に
...
: rb_define_module_under()
  ...
  で、メモリリークが起こるようになってることに注意してください。  2002/2/13 以降の修正版で直ってます。っと一度ハマッたので書いておきま
  す)。
...
: %w(...)
  ...
  いたため、以下のようなコードで異常な状態になっていました。  ((<ruby-bugs-ja:PR#91>))
...
: Subclass of String and Array
  ...
  になっていました。
        ...
        p Foo.new("").class
        p Foo.new("foo")[0,0].class              # String ???        p Foo.new("foo")[1,1].class
        ...
        p bar.push(1,2,3)
        p bar.class
        p bar[0,0].class            # => Array ???        p bar[0,1].class
        p ((bar * 5).class)
...
: $_, $~, if a..b
  ...
  ドの$_, $~が、子スレッドのもので上書きされてしまっていました。  ((<ruby-dev:14743>))
...
: CGI#header
  ...
  のように書き換えられていました。  ((<ruby-dev:14716>))
...
: ((<Digest|digest>)) モジュール
  ...
  SHA1, MD5 は Digest::SHA1, Digest::MD5 に置き換えられました。  Digest::SHA256, Digest::SHA384,  Digest::SHA512, Digest::RMD160
  も新たに追加されました。
...
: ruby -T4
  ...
  変更できないためプログラムの実行ができませんでした。  ((<ruby-dev:13401>))
...
: ((<Regexp>))#==
  すべてのオプションも同じならば同じと判断するようになりました。  以前は、漢字コード指定と /i (case-insensitive) の指定が同じで
  あれば同じと判断していました。
