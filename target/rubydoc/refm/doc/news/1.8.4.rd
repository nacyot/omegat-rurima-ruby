...
#       * sprintf.c (GETASTER): ditto.
    ...
    printfのフォーマット指定子にinteger overflowのバグがありました。    perl のそれとは違い、セキュリティバグはないそうです。    ((<URL:http://www.rubyist.net/~matz/20051207.html#p01>))
        ...
        # 1.8.4 では、上記例はすべて以下のエラーになります。        # %xx$ に指定できる最大値は 2147483647 です。        # -e:1:in `printf': width too big (ArgumentError)
...
#         [ruby-core:06721]
    ...
    フローのためにStringをキーとしたHashの検索が失敗していたバグの修正。    ((<ruby-core:06721>))
...
#
    ...
    が追加されました．    同様に Tcl/Tk8.5a3 への対応のため，テキストウィジェット上のインデック
...
#
    ...
    Ruby のソース上で tcltklib と tk とを Ruby 1.9 系と同様に一体化しました．    これにより，tcltklib の生成に失敗した環境で，動くはずのない tk のライ
    ブラリファイル群がインストールされてしまうことが避けられます．    今後はソースに含まれる tcltklib 関連のドキュメントを参照する場合には
...
#         suggested by Hiroyuki Iwatsuki.
    ...
    (/~user/public_html)以下が公開されることがなくなりました。    [webrickja:148]
