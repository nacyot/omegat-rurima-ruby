...
: instance of the user class
  ...
  _dump、_load を定義していれば 'u' になる。  インスタンス変数は dump されなくなるので、_dump/_load で対応する必要
  がある。
  ...
  marshal_dump、marshal_load を定義していれば 'U' になる。  インスタンス変数は dump されなくなるので、marshal_dump/marshal_load
  で対応する必要がある。
...
: Class/Module (old format)
  ...
  例: もはやこの形式を dump することはできないので load で例を示している。    class Mod
    end
...
: instance variable
  ...
  インスタンス変数の情報を dump しない。  ([[unknown:Marshalフォーマット/"Class/Module"]] 参照)
...
以下は、テストスクリプトです(要 [[c:RAA:RubyUnit]])
    ...
    class TestMarshal < RUNIT::TestCase
      ...
      def test_Process__Status
        assert_dumpable_and_equal system("true") && $?      end
