...
#@since 1.9.1
 [[ref:d:spec/literal#num]]。長さ 1 の文字列。#@else
 [[ref:d:spec/literal#num]]。文字の ascii コードの数値。#@end
...
: %r{/etc/httpd/logs$} や %w[foo bar baz] ・・ %<文字><区切り文字><文字列><区切り文字>
 ...
 ブレース {、小なり不等号 <、の場合は、対応する右側の括弧が終わりの <区切り文字> になります。//emlist{
...
: %!STRING!
 ...
 % 記法の一種。[[ref:d:spec/literal#percent]]。ダブルクォート文字列で %Q!STRING!と同じ。//emlist{
    p %!nomad!#=> "nomad"
//}
...
: a ||= xxx
 ...
 「||」演算子の自己代入演算子。a が 偽 か 未定義 なら a に xxx を代入する、という意味になります。//emlist{
  a ||= :some
  p a #=> some
  a ||= :sec
  p a #=> some
...
: /xx*/
 ...
 正規表現の、直前の表現の 0 回以上の繰り返し。できるだけ長くマッチしようとする。 [[d:spec/regexp]] を参照。
...
: xx ?yy : zz
 ...
 条件演算子。三項演算子とも呼ばれます。if xx then yy else zz end と同じ意味です。 [[ref:d:spec/operator#cond]] を参照。
...
: if 30 > 20 : ... など 条件式 式 :
 ...
 Ruby1.9.1 以降では使用できません。#@end

#@since 1.9.1
: { a:"aaa", b:"bbb" }

 ハッシュの新しい記法。아래와 같이 사용합니다.
//emlist{
{ :a => "aaa", :b => "bbb" }
//}
#@end
...
: 1 ... 20
 ...
 最大値を含まない Range オブジェクトを作る範囲演算子です。 全体で範囲式といいます。[[ref:d:spec/operator#range]]。
...
: 3 <=> 5
 ...
 基本的な比較演算子。ほかの比較演算子はこの演算子を元に [[c:Comparable]] モジュールで定義されています。 左が大きければ 1, 等しければ0, 右が大きければ -1 を返すように作ることが期待されています。
...
: class Foo < Super
 ...
 クラス定義でスーパークラスを指定しています。 [[ref:d:spec/def#class]]。
...
: ->(a,b){ p [a,b] }
 ...
 Ruby1.9 で導入された lambda の新しい記法。以下と同じ。//emlist{
lambda{|a, b| p [a, b] }
//}
...
: a === 12
 ...
 特殊な等号演算子。[[m:Object#===]]での説明：「このメソッドは case 文での比較に用いられます。 デフォルトは Object#== と同じ働きをしますが、 この挙動はサブクラスで所属性のチェックを実現するため 
 適宜再定義されます」。たとえば、[[m:Range#===]] はother が範囲内に含まれている時に真を返します。

: a += 12 , a *= 12 , a ||= 12 など・・a 二項演算子 = b

 自己代入演算子。[[ref:d:spec/operator#selfassign]]を参照。//emlist{
  a = 7
  a **= 2
  p a #=> 49
//}
...
: /xx$/
 ...
 正規表現で行末。文字列の末尾や改行文字の直前の位置にマッチします。改行自身は含みません。 [[d:spec/regexp]]を参照。
...
: def +@ または def -@
 ...
 単項演算子 +X や -X を定義するときの表記法。//emlist{
  class String
    def +@
      self.upcase
    end
...
: 123_456
 ...
 特別な解釈は何もしません。これは、大きな数値の桁数がひと目でわかるように記述するのに便利です。 [[ref:d:spec/literal#num]]を参照。
...
: "a is #{a}"
 ...
 式展開。[[ref:d:spec/literal#exp]]を参照。//emlist{
  a = 10
  p "a is #{a}"  #=> "a is 10"
//}
...
: "abc"
 ...
 文字列リテラル。式展開などが可能なタイプの文字列リテラルです。 [[ref:d:spec/literal#string]]を参照。
...
: `ls`
 ...
 その標準出力が文字列として与えられます。[[ref:d:spec/literal#command]]を参照。//emlist{
    puts `ruby -h`
...
: puts "abc\"def"
 ...
 文字列や正規表現の中のエスケープ。//emlist{
 puts "abc\"def" #=> abc"def
//}

: xxx \

 継続行。改行の直前に置かれる。パースの段階で直後の改行が存在しないものとして扱われます。//emlist{
    puts(3 \
    + 4) #=> 7
//}
