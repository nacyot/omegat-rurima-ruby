...
//}
  ...
  ナリかASCIIか判断が付かない場合)とはマッチしません。//emlist{
        p /./e =~ "あ"[0,1]     # => nil
//}
#@end
...
#@until 1.9.0
  ...
  チします。[0-9A-Za-z_０-９Ａ-Ｚａ-ｚ] とでも言えましょうか。#@#  ((- [2004-10-22] それじゃ、ひらがな、カタカナ、漢字は含まれていないってことにならない？実際は含むよね？/[0-9A-Za-z_０-９Ａ-Ｚａ-ｚ]/e =~ 'あ' # => nil, /\w/e =~ 'あ' # => 0 -))
#@end
...
: \G
  ...
  簡単な(あまり役に立たない)例は以下。 
//emlist{
        # 先頭から3桁ずつの数値を(数値が続く限り)取り出す。        str = "123456 789"
...
: {m,n}
  ...
  範囲指定繰り返し制御(interval quantifier)。  それぞれ直前の正規表現の
        ...
        * m 回以上
        * m 回以上、最大 n 回
  の繰り返し。{,n} や、{,} に対するマッチは必ず失敗する。//emlist{
        str = "foofoofoo"
        p str[/(foo){1}/]   # => "foo"
...
: ?
  ...
  量指定子(quantifiers)。  直前の正規表現の 0 または 1 回の繰り返し。//emlist{
...
: |
  ...
  選択(alternative)。優先順位が低いので下の「グループ化」とあわせて使うことが多い。//emlist{
...
//emlist{
        re1(?=re2)
//}
  という表現は、後に re2 とマッチする文字列が続く、正規表現 re1 です。//emlist{
        p /foo(?=bar)/ =~ "foobar"      # => 0
        p $&    # => "foo"   (bar の部分の情報はない)
//}
...
//emlist{
        (?!re1)re2
//}
  という表現は、re1 にマッチしないが re2 にはマッチする正規表現です。//emlist{
        # 000 を除く 3 桁の数字
        re = /(?!000)\d\d\d/
        p re =~ "000"   # => nil
        p re =~ "012"   # => 0
...
: (?<= )
  ...
  戻り読み(lookbehind)。  パターンによる位置指定(幅を持たない)。

//emlist{
        (?<=re)
//}
  という表現は、「直前に re にマッチする文字列が存在する位置」を表します。//emlist{
        re = /(?<=\d)/        # 直前に数字が存在する位置、つまり数字の直後
        p re =~ 'abc123def'   # => 4
        p re =~ 'abc1'        # => 4
//}
...
//emlist{
        (?<=re1)re2
//}
  という表現は、「直前に re1 にマッチする文字列が存在する位置から始まり、re2 にマッチする文字列のパターン」を表します。//emlist{
        p /(?<=baz)bar/ =~ "foobarbazbarfoo"      # => 9
        p $&    # => "bar"   (baz の部分の情報はない)
//}
//emlist{
        # 「a と c に挟まれた b のみを * に置換する」
        'abcbc'.gsub(/(?<=a)b(?=c)/, '*')  # => 'a*cbc'
//}
  戻り読みの式は固定文字長でなければなりません。//emlist{
        /(?<=\d+)/ # => invalid pattern in look-behind: /(?<=\d+)/
//}

: (?<!)

  否定戻り読み(negative lookbehind)。  パターンの否定による位置指定(幅を持たない)。

//emlist{
        (?<!re)
//}
  という表現は、「直前に re にマッチする文字列が存在しない位置」にマッチします。//emlist{
        # 文字列の先頭はその直前が \d にマッチすることはないので、必ず先頭にマッチする。        p /(?<!\d)/ =~ '123' # => 0
        p /(?<!\d)/ =~ ''    # => 0
//}
//emlist{
        (?<!re1)re2
//}
  という表現は、「直前に re1 にマッチする文字列が存在しない位置から始まり、re2 にマッチする文字列のパターン」を表します。//emlist{
        /(?<!foo)bar/ =~ 'foobarbazbarfoo'  # => 9
        p $&    # => "bar"   (foo の部分の情報はない)
//}
  否定戻り読みの式は固定文字長でなければなりません。//emlist{
        /(?<!\d+)/ # => invalid pattern in look-behind: /(?<!\d+)/
//}
#@end
...
: (?ixm-ixm)
  ...
  ンについては[[ref:d:spec/literal#regexp]]を参照。//emlist{
      re = /A(?i)a(?-i)A/
      p re =~ "AaA"         # => 0
      p re =~ "AAA"         # => 0
      p re =~ "AAa"         # => nil
...
"]" そのものを表します。#@since 1.8.0
このような "]" は、バックスラッシュエスケープを行うことが推奨されます。エスケープしない "[" や "]" に対して警告が出るようになりました。#@end
