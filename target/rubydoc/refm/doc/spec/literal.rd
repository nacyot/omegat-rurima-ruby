...
文字列はダブルクォートまたはシングルクォートで囲まれています。ダブルクォートで囲まれた文字列では[[ref:backslash]]と[[ref:exp]](後述)が有効になります。シングルクォートで囲まれた文字列では、
\\ (バックスラッシュそのもの)と \' (シングルクォート)
を除いて文字列の中身の解釈は行われません。シングルクォートで囲まれた文字列では行末の \ は \ そのものとして解釈されます。#@#[[ruby-dev:21339]]
...
として与えられます。 コマンドの終了ステータスを得るには、[[m:$?]] を
参照します。
...
子' だけの行の直前までを文字列とする行指向のリテラルです。예를 들어
...
一行に複数のヒアドキュメントを書くこともできます。
        ...
        print <<FIRST, <<SECOND
           これは一つめのヒアドキュメントです。           まだ一つめです。        FIRST
           この行からは二つめのヒアドキュメントです。           この行で終わります。        SECOND
...
以下の記法も使えます。  p :'foo-bar' #=> :"foo-bar"
  p :"foo-bar" #=> :"foo-bar"
  p %s{foo-bar} #=> :"foo-bar"

この記法では、任意のシンボルを定義することができます。#@until 1.9.1
ただし、"\0" を含めることはできません。#@end
