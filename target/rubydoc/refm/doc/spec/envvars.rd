...
: RUBYPATH
  ...
  探索対象になります。(PATH の値よりも優先します)。  起動オプションの詳細に関しては[[d:spec/rubycmd]] を参照してください。
...
#@since 1.9.1
  ...
  [[d:platform/mingw32]]版のrubyでのみ有効です。#@else
  この環境変数は [[d:platform/DJGPP]]版、[[d:platform/Cygwin]]版、[[d:platform/mswin32]]版、
  [[d:platform/mingw32]]版のrubyでのみ有効です。#@end

  この環境変数の値は、path1;path2 あるいは path1 path2 という形式で、
  Rubyライブラリの探索パス[[m:$:]]の先頭部分
  がpath1にマッチした場合に、これをpath2に置き換えます。  現在の実装ではライブラリのパスの prefix を ruby.exe や ruby.dll のあ
...
: PATH
  ...
  [[m:Kernel.#system]]などでコマンドを実行するときに検索するパスです。  設定されていないとき(nilのとき)は
