...
ラスの定義の追加になります。#@since 1.8.2
ただし、元のクラスと異なるスーパークラスを指定すると TypeError が発生します。#@else
...
とまとまりにしたり、包含関係を表すために使用されます。
        ...
        # File::RDONLY などと書かずに直接 RDONLY と書くことができる。        class File
          module Constants
             RDONLY = 0
             WRONLY = 1
          end
...
예:
          ...
          def hello    # 引数のないメソッド。            puts "Hello, world!"
          end
...
#@since 1.9.1
Ruby 1.9 では可変長引数よりも後にまだ通常の引数を置くことができます。#@end
...
例: イテレータの定義
          ...
          # 呼ばれたかどうかを判定する組み込み関数
            if block_given?              yield(1,2)
            end
          end

          # Proc.new を使う
          def bar
            if block_given?              Proc.new.call(1,2)    # proc.call(1,2) でも同じ(proc は組み込み関数)
            end
          end
...
#@since 1.8.0
特異メソッド定義式は、nil を返します。#@else
特異メソッド定義式は値を返しません。#@end
 ...
 * public に設定されたメソッドは制限なしに呼び出せます。 * private に設定されたメソッドは関数形式でしか呼び出せません。 * protected に設定されたメソッドは、そのメソッドを持つオブジェクトが
...
([[ref:d:spec/variables#builtin]] を参照)に英語名をつけます。#@end

#@until 1.8.0
グローバル変数の別名づけは特定の組み込み変数だけが対象です。#@end
