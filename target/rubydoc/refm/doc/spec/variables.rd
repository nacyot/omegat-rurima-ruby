...
すれば日本語文字の識別子も使用でき、それはローカル変数とみなされます。起動オプションの詳細に関しては[[d:spec/rubycmd]] を参照してください。#@end
...
: グローバルスコープ
  通常のグローバル変数と同じ大域的なスコープを持ちます。: ローカルスコープ
  通常のローカル変数と同じスコープを持ちます。つまり、 class 式本体やメソッド本体で行われた代入はその外側には影響しません。  プログラム内のすべての場所において代入を行わずともアクセスできることを除いて、通常のローカル変数と同じです。: スレッドローカルスコープ
