...
: -Kc
#@since 1.9.1
  外部エンコーディングとスクリプトエンコーディングをセットします。  e(EUC-JP), s(Windows-31J, CP932), u(UTF-8), n(ASCII-8BIT, BINARY) が使用できます。#@else
  Rubyの処理する漢字コードを指定します。Rubyは指定された文字
  が `E'または`e'の場合は文字列やアクセスするファイルの内容の
  コードがEUCであると仮定します。同様に`S'または`s'の場合は
  SJIS、'U'または'u'の場合はUTF-8として処理します。  `N'は漢字を処理しません。デフォルトはN(NONE)です。
...
: -e script
  ...
  挟んで解釈します。//emlist{
    以下は等価です。    ruby -e "5.times do |i|" -e "puts i" -e "end"
...
: -i[extension]
  ...
  とを指定します。元のファイルは拡張子をつけた形で保存されます。  拡張子を省略するとバックアップは行われず、変更されたファイル
...
: -r feature
  ...
  [[m:Kernel.#require]] します。  `-n'オプション、`-p'オプションとともに使う時に特に有効です。
...
//}
  ...
  に渡します。/bin/shは2行目を実行しRubyインタプリタを起動します。  Rubyインタプリタは-x
...
: -T[level]
  ...
  レベルに設定します。level 省略時は 1 を指定したのと同じです。  CGIプログラムなどでは-T1 程度を指定しておく方が良いでしょう。  [[m:$SAFE]] に指定したレベルがセットされます。

: -v
  冗長モード。起動時にバージョンの表示を行い, 組み込み変数
  [[m:$VERBOSE]]をtrueにセットします。この変数がtrueで
  ある時, いくつかのメソッドは実行時に冗長なメッセージを出力し
  합니다.`-v'オプションが指定されて, それ以外の引数がない時には
...
: -W[level]
    ...
    冗長モードを三段階のレベルで指定します。それぞれ以下の通りです。//emlist{
     * -W0: 警告を出力しない
     * -W1: 重要な警告のみ出力(デフォルト)
     * -W2 or -W: すべての警告を出力する
//}
