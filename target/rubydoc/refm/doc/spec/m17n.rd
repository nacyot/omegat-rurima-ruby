...
: ASCII互換エンコーディング
  フルサポートです。UTF-8, EUC-JP, Shift_JIS などがこれにあたります。: ASCII互換ではないエンコーディング
  スクリプトエンコーディングに使えません。またエンコーディングが固定されていない
  正規表現がマッチングを行うと例外が発生します。UTF-16LE, UTF-16BE などがこれにあたります。: ダミーエンコーディング
  文字の列としての処理をサポートしません。Ruby はエンコーディングの名前だけ知っている状態です。  ISO-2022-JP, UTF-7 がこれにあたります。
 ...
 * String のインスタンスメソッドは 1 文字ではなく 1 バイトを単位として動作します。 * エンコーディングの異なる 7bit クリーンな文字列との結合ができません。   例外 (EncodingCompatibilityError) が発生します。
 ...
 * スクリプトのリテラル中に非 ASCII 文字が含まれている場合、
   ...
   エラーになります。   -K オプションがない場合、1.8 では 1.9 の ASCII-8BIT 相当の挙動でしたが、1.9
   では US-ASCII として扱われるためです。

 * マジックコメントがあった場合、1.8 では無視されますが、1.9 ではそれ
   がスクリプトエンコーディングに反映されます。これは -K オプションよりも優先されます。

 * -K オプション・RUBYOPT・shebang の間の優先順位が 1.8 と 1.9 では違います。   それぞれの優先順位は以下の通りです。左が優先です。//emlist{
