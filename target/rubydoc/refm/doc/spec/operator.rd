...
//emlist{
    ...
    +  -  *  /    %   **   ~   +@  -@  []  []=  `
//}
  これらの演算子式の定義方法については[[ref:d:spec/def#operator]]を参照してください。#@end
#@since 1.9.0
//emlist{
    |  ^  &  <=>  ==  ===  =~  >   >=  <   <=   <<  >>
    +  -  *  /    %   **   ~   +@  -@  []  []=  ` !!= !~
...
#@end
#@since 1.9.0
  演算子の組合せである自己代入演算子は再定義できません。//emlist{
    =  ?:  ..  ...  not  &&  and  ||  or  ::
//}
#@end
...
#@#((- foo = 1, 2, 3 という書き方は 1.6.2 あたりから可能なようです。#@#おそらくブロックパラメータの受渡しとの対称性からかと思います
...
「..」の場合:
 ...
 (1) 初期状態では式1だけを評価し、式1が真を返すまでは false を返します。 (2) 式1が真を返すと true を返します。式2が真なら初期状態に戻ります。 (3) この後は式2だけを評価し、式2が真を返すまで true を返します。 (4) 式2が真を返すと true を返したあと、初期状態に戻ります。

「...」の場合:
 (1) 初期状態では式1だけを評価し、式1が真を返すまでは false を返します。 (2) 式1が真を返すと true を返します。 (3) この後は式2だけを評価し、式2が真を返すまで true を返します。 (4) 式2が真を返すと true を返したあと、初期状態に戻ります。
