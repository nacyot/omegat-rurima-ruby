...
の処理が順に実行されます。
  ...
  (1) すべてのスレッドを [[m:Thread.kill]] する。  (1) Ruby の疑似シグナル SIGEXIT のハンドラが登録されていればそれを実
      行する([[m:Kernel.#trap]] を参照)。  (1) [[ref:d:spec/control#END]] ブロック(END { ... } または関数
      [[m:Kernel.#at_exit]] で指定したブロック)が登録されていれば、
      そのブロックを登録とは逆順に実行する。このブロックの実行中に発生
      した大域脱出はそのブロックの処理を中断するが。スクリプトはまだ終
      了しない。  (1) [[m:ObjectSpace.#define_finalizer]] により、ファイナ
      ライザが登録されていればそれらを実行する。実行順序は不定。      ファイナライザ実行中に発生した大域脱出はそのファイナライザの処理
      を中断するが、スクリプトはまだ終了しない。  (1) [[man:exit(3)]] により終了する。      このとき渡される終了ステータス値は終了方法によります。      예를 들어
//emlist{
      * スクリプトの終端に達した場合は 0
