...
//emlist{
      ...
      * 標準入力からのプログラム読み込みを行わない
        (スクリプトがsetgid, setuidされている時も同様)
//}
   * setuid, setgid されたスクリプトは $SAFE = 1 以上で実行される。   * [[c:Proc]] はその時点でのセーフレベルを記憶する。     その [[c:Proc]] オブジェクトが call されると、記憶していたセーフレベルで実行される。   * 汚染された [[c:Method]] オブジェクトが call されるとレベル4で実行される。#@since 1.8.0
   * 汚染された文字列を第二引数に指定して [[m:Kernel.#trap]]/[[m:Kernel.#trace_var]] を
     実行するとその時点で例外 [[c:SecurityError]] が発生する。#@else
   * 汚染された文字列を [[m:Kernel.#trap]]/[[m:Kernel.#trace_var]] の第二引数に指定するとレベル4で実行される。#@end
   * レベル4以上では out of memory でも [[c:fatal]] にならない。   * 実装の都合上 [[c:Fixnum]], [[c:Symbol]], true, false, nil は汚染されない。     なお [[c:Bignum]], [[c:Float]] は汚染されることは注意が必要。
 ...
 * 拡張ライブラリではオブジェクトの汚染状態を適切に伝播させる必要があります。 * グローバルな状態を変更する場合や外部とのやりとりの前にセキュリティレベルを
   チェックする必要があります。
