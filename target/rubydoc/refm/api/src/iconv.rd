...
이 라이브러리는 더 이상 사용되지 않습니다.2.0 以降では利用することが出来ません。#@since 1.9.1
代わりに [[m:String#encode]] を利用してください。#@end
...
: //TRANSLIT
  ...
  文字列の変換時に、表現できない文字を同じ見た目の文字に"翻訳"します。#@since 1.9.1
  [[m:Iconv#transliterate=]] に真を渡して指定する事もできます。#@end

: //IGNORE

  文字列の変換時に、 [[c:Iconv::IllegalSequence]] が発生するような文字列
  が途中にあった場合でも無視して変換を継続します。#@since 1.9.1
  [[m:Iconv#discard_ilseq=]] に真を渡して指定する事もできます。#@end

ただし、上記の拡張は移植性を損ないます。#@since 1.9.1
そのような機能が必要な場合は [[m:String#encode]] を使用してください。#@else
そのため、使用は避けてください。#@end
...
@param options ハッシュ形式で transliterate、discard_ilseq に真か偽かを
               与えて[[ref:lib:iconv#gnu_options]] と同じ操作を行います。#@end
...
@raise NotImplementedError 実行プラットフォームでサポートされていないオ
                           プションを指定した場合に発生します。#@end

예:
  require 'iconv'
  # EUC-JP から SHIFT_JIS へ変換するIconvオブジェクトを生成。  icv = Iconv.new('SHIFT_JIS', 'EUC-JP')
...
次の省略形です。  Iconv.open(to, from) {|cd| (strs + [nil]).collect {|s| cd.iconv(s)}}
...
次の省略形です。  Iconv.iconv(to, from, str).join
...
@raise NotImplementedError 実行プラットフォームでサポートされていない場合に発生します。#@end
...
변환을 종료합니다.出力バッファを初期シフト状態に戻すための文字列を返します。出力の文字符号化方式が内部状態をも持たない場合、空文字列を返します。
...
예:
  ...
  iconv = Iconv.new('EUC-JP', 'UTF-8') # UTF-8 에서 EUC 로 변환
  str = iconv.conv(utf8)
  puts str #=> "あいうえお"
...
メソッドの引数等で指定された文字コード体系が見つからない (システム上で有効でない) 場合に発生します。#@end
