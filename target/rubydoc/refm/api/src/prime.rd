...
: [[c:Prime::EratosthenesGenerator]]
  エラトステネスの篩いを使用します。: [[c:Prime::TrialDivisionGenerator]]
  試行除算法を使用します。: [[c:Prime::Generator23]]
  2 と 3 で割り切れない全ての正の整数を生成します。  この数列は素数の数列としては使い物になりません。しかし、他の生成器より速く、
...
Prime 클래스는 싱글톤이라고 생각해주세요.Prime クラスはデフォルトのインスタンスを持っており、ユーザーはそのインスタンスを利用すべきです。[[m:Prime.instance]] によってそのインスタンスを取得できます。
...
@param upper_bound 任意の正の整数を指定します。列挙の上界です。                   nil が与えられた場合は無限に列挙し続けます。

@param generator 素数生成器のインスタンスを指定します。

@return ブロックの最後に評価された値を返します。        ブロックが与えられなかった場合は、[[c:Enumerator]] と互換性のある外部イテレータを返します。
...
このメソッドは、素数列の外部イテレータを内部イテレータに変換してRubyらしいプログラミングを提供することが責務です。独自に素数性の保障するのはメソッドの責務ではありません。従って、次のように精度の低い素数生成器を与えると、真に素数とは限らない数列が発生します。 Prime.each(50, Prime::Generator23.new) do |n|
...
結果は  p_1**e_1 * p_2**e_2 * .... * p_n**e_n이 됩니다.
...
与えられた整数が素数である場合は、真を返します。그렇지 않으면 false를 리턴합니다.
...
@return 最初に与えられたオブジェクトを返します。@return ブロックを与えられた場合は obj を返します。ブロックを与えられなかった場合は Enumerator を返します。
...
@return ブロック付きで呼び出された場合は break の引数が返り値になります。        ブロック無しで呼び出された場合は [[c:Prime::EratosthenesGenerator]] のインスタンスを返します。
...
@param upper_bound 任意の正の整数を指定します。列挙の上界です。                   nil が与えられた場合は無限に列挙し続けます。@return ブロックの最後に評価された値を返します。        ブロックが与えられなかった場合は、[[c:Enumerator]] と互換性のある外部イテレータを返します。
