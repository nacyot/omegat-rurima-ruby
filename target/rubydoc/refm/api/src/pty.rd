...
@return 返値は3つの要素からなる配列です。最初の要素は擬似 tty から
        ...
        3番目の要素は子プロセスのプロセス ID です。        このメソッドがブロック付き呼ばれた場合、これらの要素はブロックパラメータとして渡され、
        メソッド自体は nil を返します。

#@until 1.9.1
@raise PTY::ChildExited 子プロセスが終了したり停止したりした場合に発生します。#@end
#@if(version == "1.9.1")
...
#@until 1.9.2
--- protect_signal{ ... } -> self
このメソッドは何もしません。이 메소드는 더 이상 사용되지 않습니다.
...
@param raise 真を指定すると、子プロセスが終了または停止していた場合、
             例外 [[c:PTY::ChildExited]] が発生します。기본 설정은 false입니다.
...
子プロセスの終了ステータスを[[c:Process::Status]]オブジェクトで返します。#@end
