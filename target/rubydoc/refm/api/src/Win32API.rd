...
=== 使用例 3: Cygwin の cygpath コマンドの代わり
  ...
  module Cygwin
    ...
    def cygpath(options, path)
      ...
      raise ArgumentError "first argument must contain -u or -w" if func.nil?      func = absolute ?func[0] : func[1]
      buf = "\0" * 300
...
API를 호출합니다.
指定する引数と戻り値は [[m:Win32API.new]] の引数の指定に従います。特にポインタを渡してそのポインタの指す領域に値が設定される場合は
