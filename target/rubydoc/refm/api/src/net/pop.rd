...
[[RFC:2449]] で定義されているPOP3拡張には対応していません。=== 예
...
に適宜読みかえてください。
  ...
  if pop.mails.empty?    $stderr.puts 'no mail.'
  else
...
以下のコードと同じ動作をします。  Net::POP3.new(address, port, isapop).start(account, password)
...
以下のコードと同様の処理をします。  Net::POP3.start(address, port, account, password, isapop=false) {|pop|
    pop.each_mail do |m|
      yield m
    end
  }
...
--- Revision -> String
ライブラリ(ファイル)のリビジョンです。사용하지 마시기 바랍니다.
