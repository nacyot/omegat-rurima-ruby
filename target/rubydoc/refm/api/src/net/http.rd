...
범용 데이터 전송 프로토콜 HTTP를 다루는 라이브러리입니다.
구현은 [[RFC:2616]]에 기초합니다.
...
[[m:Net::HTTP.Proxy]]는 프록시를 사용해 접속하는 클래스를
생성해서 리턴합니다.이 클래스는 [[c:Net::HTTP]]와 같은 
메소드를 가지며 같은 방식으로 작동합니다.차이점은
모든 데이터 전송이 프록시를 경유해서 이루어지는 점입니다.
  require 'net/http'
...
オプション引数があり、以下のようにして
プロクシの認証をすることができます。  proxy_host = 'your.proxy.host'
  proxy_port = 8080
...
通常この機能は使わないはずです。1.2固定で利用してください。#@end
...
@param address 接続するホスト名を文字列で指定します。@param port 接続するポート番号を指定します。@param proxy_addr プロクシのホスト名を指定します。省略した場合には直接接続します。@param proxy_port プロクシのホスト名を指定します。@param proxy_user プロクシの認証のユーザ名を指定します。省略した場合には認証はなされません。@param proxy_pass プロクシの認証のパスワードを指定します。@see [[m:Net::HTTP.new]], [[m:Net::HTTP#start]]
...
@param uri データの取得対象を [[c:URI]] で指定します。@param host 接続先のホストを文字列で指定します。@param path データの存在するパスを文字列で指定します。@param port 接続するポートを整数で指定します。@see [[m:Net::HTTP#get]]
...
そうでないなら nil を返します。@see [[m:Net::HTTP.Proxy]]
...
@param second 待つ秒数を指定します。@see [[m:Net::HTTP#read_timeout]], [[m:Net::HTTP#open_timeout]]
...
@param second 待つ秒数を指定します。@see [[m:Net::HTTP#open_timeout]], [[m:Net::HTTP#read_timeout]]
...
@raise ArgumentError dataを与えるべきでないリクエストでdataを
                     与えた場合に発生します。@see [[m:Net::HTTP#send_request]]
...
@param path リクエストを送るパスを文字列で与えます。@param body リクエストのボディを文字列で与えます。@param initheader リクエストのヘッダを「文字列=>文字列」の
                  ハッシュで与えます。@see [[c:Net::HTTP::Unlock]]
...
@param key ヘッダフィール名を文字列で与えます。@param val keyで指定したフィールドに追加する文字列を与えます。@see [[m:Net::HTTPHeader#[] ]], [[m:Net::HTTPHeader#[]=]],
     [[m:Net::HTTPHeader#get_fields]]
...
@param key ヘッダフィール名を文字列で与えます。@see [[m:Net::HTTPHeader#[] ]], [[m:Net::HTTPHeader#[]=]],
     [[m:Net::HTTPHeader#add_field]],
...
@param key ヘッダフィール名を文字列で与えます。@param default 該当するキーが登録されていない時の返り値を指定します。@raise IndexError 引数defaultもブロックも与えられてない時、キーの探索に 失敗すると発生します。@see [[m:Net::HTTPHeader#[] ]]
...
@return 取り除かれたフィールドの値を返します。        key ヘッダフィールドが存在しなかった場合には
        nil을 반환힙니다.
...
以下は同じことを表しています。  req.range = 0..1023
  req.range = 0...1024
  req.range = 1024
