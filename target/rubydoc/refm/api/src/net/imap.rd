...
이 라이브러리는 Internet Message Access Protocol (IMAP)의 클라이언트 라이브러리입니다.[[RFC:2060]]를 따릅니다.

=== IMAP의 개요

IMAP을 사용하기 위해선 우선 서버에 접속하고
[[m:Net::IMAP#authenticate]] 혹은 
[[m:Net::IMAP#login]] 메소드를 통해 인증을 받아야합니다.
IMAP에선 메일 박스라는 개념이 중요합니다.
メールボックスは階層的な名前を持ちます。各メールボックスはメールを保持することができます。メールボックスの実装はサーバソフトウェアによって異なります。Unixシステムでは、ディレクトリ階層上の
...
デフォルトのメールボックス(INBOX)の送り元とサブジェクトを表示する。  imap = Net::IMAP.new('mail.example.com')
...
: NO
  コマンドが正常に完了しなかったことを意味します。  例えば、ログインでのユーザ名/パスワードが間違っていた、
  選択したメールボックスが存在しない、などです。

: BAD
  クライアントからのリクエストをサーバが理解できなかった
  ことを意味します。  クライアントの現在の状態では使えないコマンドを使おうとした
  場合にも発生します。예를 들어
  selected状態(SELECT/EXAMINEでこの状態に移行する)にならずに
  SEARCH コマンドを使おうとした場合に発生します。  サーバの内部エラー(ディスクが壊れたなど)の場合も
  このエラーが発生します。

: BYE
  サーバが接続を切ろうとしていることを意味します。  これは通常のログアウト処理で発生します。  また、ログイン時にサーバが(なんらかの理由で)接続
  したくない場合にも発生します。  それ以外では、サーバがシャットダウンする場合か
...
という例外クラスに対応しています。原理的には、これらの例外はサーバにコマンドを送った場合には
...
#@since 1.9.1
パラメータは Hash で渡すこともできます。以下のキーを使うことができます。  * :port ポート番号
    省略時は SSL/TLS 使用時→993 不使用時→143 となります。  * :ssl OpenSSL に渡すパラメータをハッシュで指定します。    省略時は SSL/TLS を使わず接続します。    これで渡せるパラメータは
    [[m:OpenSSL::SSL::SSLContext#set_params]] と同じです。これの :ssl パラメータを使うことで、OpenSSL のパラメータを詳細に
調整できます。
...
auth_type としては以下がサポートされています。  * "LOGIN"
  * "PLAIN"
  * "CRAM-MD5"
  * "DIGEST-MD5"
...
@raise Net::IMAP::NoResponseError 指定した名前のメールボックスを
       リネームできなかった場合に発生します。       指定した名前のメールボックスが存在しない場合や、
...
@raise Net::IMAP::NoResponseError 指定した名前のメールボックスを
       追加できなかった場合に発生します。       指定した名前のメールボックスが存在しない場合などに
       生じます。
...
@raise Net::IMAP::NoResponseError 指定した名前のメールボックスを
       削除できなかった場合に発生します。       指定した名前のメールボックスが active/subscribe でなかった
       場合などに発生します。
...
attr で何をどのように変化させるかを指定します。以下を指定することができます。  * "FLAGS"
  * "+FLAGS"
  * "-FLAGS"
それぞれメッセージのフラグの置き換え、追加、削除を意味します。詳しくは [[RFC:2060]] の 6.4.6 を参考にしてください。
...
algorithm は木構造を決定するためのアルゴリズムを指定します。以下の2つが利用可能です。  * "ORDEREDSUBJECT" subjectを使って平坦に区切るだけ
...
#@else
このメソッドは obsolete です。사용하지 마시기 바랍니다.
#@end
...
この配列には例えば以下のような値を含んでいます。詳しくは [[RFC:2060]] 7.2.2 などを参照してください。以下のもの以外で、IMAP 関連 RFC で拡張された値を含んでいる
場合もあります
  * :Noselect
  * :Noinferiors
  * :Marked
...
: BODY
    BODYSTRUCTURE の拡張データなしの形式。    [[c:Net::IMAP::BodyTypeBasic]], [[c:Net::IMAP::BodyTypeText]],
    [[c:Net::IMAP::BodyTypeMessage]], [[c:Net::IMAP::BodyTypeMultipart]]
    のいずれか。: BODY[<section>]<<partial>>
    section で指定されたセクションのボディの内容。文字列。: BODY.PEEK[<section>]<<partial>>
    section で指定されたセクションのメッセージボディの内容。文字列。    ただしこれで内容を見ても :Seen フラグを設定しない点が
    BODY[<section>]と同様
: BODYSTRUCTURE
    MIME-IMB でのメッセージボディ。    [[c:Net::IMAP::BodyTypeBasic]], [[c:Net::IMAP::BodyTypeText]],
    [[c:Net::IMAP::BodyTypeMessage]], [[c:Net::IMAP::BodyTypeMultipart]]
    のいずれか。: ENVELOPE
    メッセージのエンベロープ。    [[c:Net::IMAP::Envelope]] オブジェクト。: FLAGS
    メッセージにセットされたフラグ。    [[c:Symbol]] の配列。[[m:String#capitalize]] でキャピタライズ
    されている。: INTERNALDATE
    メッセージの内部日付。文字列。: RFC822
    BODY[] と同じ。文字列。: RFC822.HEADER
    BODY.PEEK[HEADER] と同じ。文字列。: RFC822.SIZE
    メッセージの [[RFC:822]] サイズ。整数。: RFC822.TEXT
    BODY[TEXT] と同じ。文字列。: UID
    UID。整数。
