...
syslog の詳細については [[man:syslog(3)]] を参照してください。
  ...
  Syslog.close
  # 書き込まれているか確かめる。  # 但し、実行環境によってログの場所が違う。くわしくはsyslog.confを参照。  File.foreach('/var/log/system.log'){|line|
    line.chomp!    if /syslogtest/ =~ line
      puts line
    end
  }
...
@param options Syslog.open や Syslog.log の動作を制御するフラグを指定します。               指定しない場合は、Syslog::LOG_PID|Syslog::LOG_CONSの値が使われ
               합니다.使用できる値は[[c:Syslog::Constants]] を参照してください。               
@param facility ログ出力を行うプログラムの種別を指定します。syslog はこの値
                にしたがって出力先となるログファイルを決定します。자세한 내용은
                [[man:syslog.conf(5)]], 
...
@param facility ログ出力を行うプログラムの種別を指定します。syslog はこの値
                にしたがって出力先となるログファイルを決定します。자세한 내용은
                [[man:syslog.conf(5)]], [[c:Syslog::Constants]] 를 참조해
                주세요.
...
@param priority priority は優先度を示す定数を指定します。                詳しくは、[[c:Syslog::Constants]]を参照してください。
