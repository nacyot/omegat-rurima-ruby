...
#@since 1.8.2
セッション情報ファイルは 0600 で作成されるようになりました。#@end
  ...
  <HTML>
    <BODY>
      <FORM METHOD="post" ENCTYPE="application/x-www-form-urlencoded" action="/sample.rb">
        <P>
          あなたの名前は？          <INPUT NAME="name" SIZE="40" TYPE="text">
...
ソースコード
  ...
  class SessionDemo
    def initialize
      ...
      @session = CGI::Session.new(@cgi)               # セッションはこうして生成する。      @cmd = "#{@cgi['cmd'].first}"                   # ruby 1.8 でも動くように(warning は出ます)
      @cmd = 'start' if @cmd.empty?      @header = { "type" => "text/html", "charset" => "euc-jp" }
    ...
    def cmd_start
      @cgi.out(@header) {
        ...
        <body>
         ...
         <p>
         あなたの名前は？         <input type="text" name="name">
...
: session_id
  セッション ID として使われます。  デフォルトのデータベースである FileStore を用いる場合,
  値は英数字だけからなる文字列で無ければなりません。  このオプションを指定するとリクエストにセッション ID が含まれても無視します。  (default: ランダムに生成されます)

: new_session
    値が true のときは強制的に新しいセッションを始めます。#@since 1.8.2
    値が false のときは、リクエストにセッション ID が含まれていない場合に
    例外 [[c:ArgumentError]] が発生します。    値がないときは、リクエストにセッション ID が
    含まれている場合はそれを使用し、含まれていない場合は新しいセッションを始めます。#@end
    (default: 偽)

: database_manager
  データベースクラスを指定します。  組み込みで [[c:CGI::Session::FileStore]], [[c:CGI::Session::MemoryStore]],
  [[c:CGI::Session::PStore]] を提供しています。デフォルトは [[c:CGI::Session::FileStore]] です。

: session_expires
    セッションの有効期間。    [[c:Time]] オブジェクトを与えると、セッションはその日時まで破棄されずに残ります。    デフォルトでは、セッションはブラウザの終了と同時に破棄されます。
...
: tmpdir
    [[c:CGI::Session::FileStore]] がセッションデータを作成するディレクトリの名前を指定します。    デフォルトは [[m:Dir.tmpdir]] です。

: prefix
    [[c:CGI::Session::FileStore]] がセッションデータのファイル名に与えるプレフィックス。    (default: "")

#@since 1.8.2
: suffix
    [[c:CGI::Session::FileStore]] がセッションデータのファイル名に与えるサフィックス。    (default: "")
#@end
...
: tmpdir
    セッションデータを作成するディレクトリの名前を指定します。    デフォルトは [[m:Dir.tmpdir]] です。
...
#@since 1.8.2
: suffix
    セッションデータのファイル名に与えるサフィックスを指定します。    デフォルトは空文字列です。#@end
