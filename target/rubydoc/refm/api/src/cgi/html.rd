#@since 1.9.1
HTML 태그를 만드는 메소드를 제공하는 라이브러리입니다.
...
블록을 넘기면 블록을 평가한 결과를 내용으로 합니다.

@param href 문자열로 링크 주소를 지정합니다.속성들을 hash로 지정할 수도 있습니다.
...
@param cite 출처를 지정합니다.속성들을 hash로 지정할 수도 있습니다.
...
@param align 문자열 정렬을 지정합니다.(top, bottom, left, right를 지정할 수 있습니다.)
             속성들을 hash로 지정할 수도 있습니다.
...
타입가 checkbox인 input 요소를 생성합니다.

@param name name 속성의 값을 지정합니다.

@param value value 속성의 값을 지정합니다.

@param checked checked 속성의 값을 지정합니다.
...
@param attributes 속성을 해쉬로 지정합니다.
...
타입이 checkbox인 input 요소 그룹을 생성합니다.

생성된 input 요소의 name 속성은 전부 똑같이 생성되며
각각의 input @@@

@param name name 속성의 값을 지정합니다.

@param values value 속성의 리스트를 지정합니다.
              それぞれの引数が、単純な文字列の場合、value 属性の値とラベルに同じものが使用されます。              それぞれの引数が、二要素または三要素の配列の場合、最終要素が true であれば、
...
form 要素を生成します。블록을 넘기면 블록을 평가한 결과를 내용으로 합니다.
...
--- html(attributes = {}){ ... } -> String
トップレベルの html 要素を生成します。블록을 넘기면 블록을 평가한 결과를 내용으로 합니다.

@param attributes 속성을 해쉬로 지정합니다.
                  擬似属性の "PRETTY" に文字列を与えるとその文字列でインデントした HTML を生成します。                  擬似属性の "DOCTYPE" には DOCTYPE 宣言として使用する文字列を与えることができます。
...
enctype 属性に "multipart/form-data" をセットした form 要素を生成します。블록을 넘기면 블록을 평가한 결과를 내용으로 합니다.
...
@param values option 要素を生成するための情報を一つ以上指定します。              それぞれ、文字列、一要素、二要素、三要素の配列を指定することができます。              文字列か一要素の配列である場合は、value 属性の値と option 要素の内容になります。              三要素の配列である場合は、順に value 属性の値、option 要素の内容、その option 要素が
              選択状態かどうかを表す真偽値となります。       
        예 :
