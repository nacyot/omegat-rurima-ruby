...
@param flags flags には、[[m:GDBM::FAST]], [[m:GDBM::SYNC]], [[m:GDBM::NOLOCK]]
             의 논리합을 지정합니다.デフォルト値は指定なし(つまり0)です。#@if (version >= "1.8.2")
             flags に [[m:GDBM::READER]], [[m:GDBM::WRITER]], [[m:GDBM::WRCREAT]], [[m:GDBM::NEWDB]]
             のいずれかを与えて読み書きのモードを指定できます。             これらをどれも指定しなかった場合には、
             [[m:GDBM::WRCREAT]], [[m:GDBM::WRITER]], [[m:GDBM::READER]] の順で試します。#@end
...
DBM ファイルが既に閉じられている場合は、真を返します。그렇지 않은 경우엔 false를 반환합니다.
...
@return 指定したキーが存在する場合は、キーに対応する値を返します。        指定したキーが存在しない場合は、 nil を返します。        また、キーが存在しない場合にブロックを与えている場合は、ブロックを評価した結果を返します。
...
書き込みモードで、すでにファイルが存在したら削除してから作り直します。#@end
