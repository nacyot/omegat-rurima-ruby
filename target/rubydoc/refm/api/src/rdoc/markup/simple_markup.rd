...
예:
  ...
  class WikiHtml < RDoc::Markup::ToHtml
    # WikiWord のフォントを赤く表示。    def handle_special_WIKIWORD(special)
      "<font color=red>" + special.text + "</font>"
    end
  end

  m = RDoc::Markup.new
  # { 〜 } までを :STRIKE でフォーマットする。  m.add_word_pair("{", "}", :STRIKE)
  # <no> 〜 </no> までを :STRIKE でフォーマットする。  m.add_html("no", :STRIKE)

  # WikiWord を追加。  m.add_special(/\b([A-Z][a-z]+[A-Z]\w+)/, :WIKIWORD)

  wh = WikiHtml.new(m)
  # :STRIKE のフォーマットを <strike> 〜 </strike> に指定。  wh.add_tag(:STRIKE, "<strike>", "</strike>")

  puts "<body>#{wh.convert ARGF.read}</body>"

変換する形式を変更する場合、フォーマッタ(例. [[c:RDoc::Markup::ToHtml]])
を変更、拡張する必要があります。#@else
예:
...
예:
  ...
  h = WikiHtml.new
  # :STRIKE のフォーマットを <strike> 〜 </strike> に指定。  h.add_tag(:STRIKE, "<strike>", "</strike>")
...
空白文字です。?\s を返します。라이브러리 내부에서 사용합니다.

--- SIMPLE_LIST_RE -> Regexp

リストにマッチする正規表現です。라이브러리 내부에서 사용합니다.

ラベルの有無を問わずマッチします。

--- LABEL_LIST_RE -> Regexp

ラベル付きリストにマッチする正規表現です。라이브러리 내부에서 사용합니다.
...
自身の [[c:RDoc::AttributeManager]] オブジェクトを返します。#@end
