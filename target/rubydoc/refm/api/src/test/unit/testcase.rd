...
テストの基本単位(あるいは「テスト本体」)を表すクラスです。テストを行うメソッド(テストメソッド)は TestCase のサブクラスのインスタンスメソッド
として定義されます。テストメソッドの名前は「test」で始まっていなければなりません。逆に、「test」で始まっているメソッドは全てテストメソッドと見なされます。#@until 1.9.1
各テストメソッドは、[[m:Test::Unit::TestCase.suite]] により [[c:Test::Unit::TestSuite]]
オブジェクトへとひとつにまとめられます。#@end
...
テストメソッドの数だけ TestCase オブジェクトが生成されます。#@until 1.9.1
テストの実行時にはそれぞれの結果が[[c:Test::Unit::TestResult]]に集計されます。#@end
