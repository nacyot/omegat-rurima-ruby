...
Rubyのオブジェクトを外部ファイルに格納するためのクラスです。내부에서 [[c:Marshal]]를 사용합니다.
...
file のあるディレクトリは書き込み可能である必要があります。#@since 1.8.2
データベースの更新が成功すると、バックアップファイルは削除されます。バックアップファイル名は
ファイル名に ".tmp" および ".new" を付けたものです。#@else
バックアップファイルは削除されずに残ります。バックアップファイル名はファイル名の後に "~" を付けたものです。#@end

@param file データベースファイル名。

#@since 1.9.1
@param thread_safe 真を指定すると [[c:Mutex]] を用いてスレッドセーフになります。기본 설정은 false입니다.
#@end
== Instance Methods
...
#@since 1.9.1
--- ultra_safe -> bool
真であれば、パフォーマンスと引き換えにファイル更新の衝突を避けることができます。기본 설정은 false입니다.
