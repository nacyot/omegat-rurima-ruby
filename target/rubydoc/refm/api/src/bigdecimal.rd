...
浮動小数点構造体 (Real) は以下のようになっています。
  ...
  typedef struct {
     ...
     short    sign;         // 以下のように符号等の状態を定義します。                            //  ==0 : NaN
                            //    1 : +0
                            //   -1 : -0
                            //    2 : 正の値
                            //   -2 : 負の値
...
c = a op b という計算 (op は + - * /) をしたときの動作は以下のようになります。
  ...
  (1) 乗算は (a の有効桁数) + (b の有効桁数)、
      ...
      (実際は、余裕を持って、もう少し大きくなります) を持つ変数 c を新たに生成します。      加減算の場合は、誤差が出ないだけの精度を持つ c を生成します。      例えば c = 0.1+0.1*10**(-100) のような場合、c の精度は100桁以上の精度を持つようになります。  (2) 次に c = a op b の計算を実行します。
...
以下のメソッド以外にも、(C ではない) Ruby ソースの形で提供されているものもあります。예를 들어
