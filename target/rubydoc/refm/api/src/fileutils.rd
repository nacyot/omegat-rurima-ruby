...
: :noop
  真を指定すると実際の処理は行いません。: :preserve
  真を指定すると更新時刻と、可能なら所有ユーザ・所有グループもコピーします。: :verbose
  真を指定すると詳細を出力します。: :mode
  パーミッションを8進数で指定します。: :force
  真を指定すると作業中すべての [[c:StandardError]] を無視します。: :nocreate
  真を指定するとファイルを作成しません。: :dereference_root
  真を指定すると src についてだけシンボリックリンクの指す
  内容をコピーします。偽の場合はシンボリックリンク自体をコピーします。: :remove_destination
  真を指定するとコピーを実行する前にコピー先を削除します。: :secure
  真を指定するとファイルの削除に [[m:FileUtils.#remove_entry_secure]] を使用します。: :mtime
...
#@since 1.8.3
@param options :verbose が指定できます。#@else
@param options :noop, :verbose が指定できます。#@end
...
@param options :noop と :verbose が指定可能です。               [[ref:c:FileUtils#options]]
...
@param src コピー元。一つの場合は文字列でも指定可能です。           二つ以上指定する場合は配列で指定します。

@param dest コピー先のファイルかディレクトリです。

@param options :preserve, :noop, :verbose が指定できます。               [[ref:c:FileUtils#options]]
...
@param options :preserve, :noop, :verbose, :dereference_root, :remove_destination が指定できます。               [[ref:c:FileUtils#options]]
...
@param options :preserve, :noop, :verbose, :mode が指定できます。               [[ref:c:FileUtils#options]]
...
@param src リンク元。一つの場合は文字列でも指定可能です。           二つ以上指定する場合は配列で指定します。

@param dest リンク作成先のファイルかディレクトリです。

@param options :force, :noop, :verbose が指定できます。               [[ref:c:FileUtils#options]]
...
@param options :mode, :noop, :verbose が指定できます。               [[ref:c:FileUtils#options]]
...
@param list 作成するディレクトリ。一つの場合は文字列でも指定できます。            二つ以上指定する場合は配列で指定します。

@param options :mode, :noop, :verbose が指定できます。               [[ref:c:FileUtils#options]]

@return ディレクトリ名文字列の配列を返します。#@until 1.9.1
...
@param src 元のファイル。一つの場合は文字列でも指定可能です。           二つ以上指定する場合は配列で指定します。

@param dest 移動先のファイル、またはディレクトリ。

@param options :force, :noop, :verbose, :secure が指定できます。               [[ref:c:FileUtils#options]]
...
@param list 削除する対象。一つの場合は文字列も指定可能です。            二つ以上指定する場合は配列で指定します。
...
@param options :noop, :verbose が指定できます。               :force がセットされた場合は作業中すべての [[c:StandardError]] を無視します。
...
@param options :noop, :verbose, :secure が指定できます。               [[ref:c:FileUtils#options]]
...
@param dir 削除するディレクトリを指定します。一つの場合は文字列でも指定可能です。           二つ以上指定する場合は配列で指定します。

@param options :noop, :verbose が指定できます。               [[ref:c:FileUtils#options]]
...
仮定しています。
  ...
  * UNIX システムおよびそれに類する環境では、sticky ビットが立っていること。  * 全ユーザが書き込み可能であるのは、直接の親ディレクトリのみであること。    例えば、/var/tmp のパーミッションが 1777 であるのは問題ありませんが、
...
@param list 対象のファイル。一つの場合は文字列も指定可能です。            二つ以上指定する場合は配列で指定します。

@param options :mtime, :nocreate, :noop, :verbose が指定できます。               [[ref:c:FileUtils#options]]
...
mid というメソッドが opt というオプションを持つ場合、真を返します。그렇지 않은 경우엔 false를 반환합니다.
...
#@# nodoc
#@# name で指定されたメソッドをモジュール関数にします。#@# また、可視性を private にします。#@# 
