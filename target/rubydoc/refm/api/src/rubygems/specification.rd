...
RDoc を生成するかどうかをセットします。기본 설정은 false입니다.
...
真の場合はユニットテストを含みます。그렇지 않으면 false를 리턴합니다.
...
既にロードされている場合は真を返します。그렇지 않으면 false를 리턴합니다.
...
@param platform この Gem パッケージを使用できるプラットフォームを指定します。       デフォルトは [[m:Gem::Platform::RUBY]] です。
...
依存関係を満たす場合は真を返します。그렇지 않으면 false를 리턴합니다.
 ...
 * クラス変数 @@attributes, @@default_value を変更します。 * 通常の属性書き込みメソッドを定義します。 * デフォルト値を持つ属性読み取りメソッドのように振る舞うメソッドを定義します。
...
예:
  ...
  # こう書くことができます。  s.require_path = 'mylib'
