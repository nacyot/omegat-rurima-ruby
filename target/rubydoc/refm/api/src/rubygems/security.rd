...
: NoSecurity
  ...
  このようなセキュリティを簡単に回避することができます。: MediumSecurity
  LowSecurity, NoSecurity よりも良いですが、まだ不確実です。  パッケージの内容は、署名付きの証明書に対して検証されます。  証明書の正当性が検証されます。証明書は残りの証明書チェーンに対して検証されます。  (あなたが証明書チェーンについて知らない場合は、すぐに説明するので少し待ってください)
  LowSecurity ポリシーからの最大の改善点は MediumSecurity ポリシーは信頼出来ない
  ソースが署名したパッケージをインストールしないことです。  不幸なことに MediumSecurity ポリシーは完全にセキュアというわけではありません。  悪意のあるユーザは、署名を外したり、署名なしの Gem を配布すると、 Gem を展開する
  ことができます。: HighSecurity
  この厄介事は私たちをこんな状態にしました。HighSecurity ポリシーは
  署名なしの Gem をインストールしないということを除いて MediumSecurity と同じです。  悪意のあるユーザは、なんの手段もも持っていない。悪意のあるユーザは、署名を
...
RubyGems が魔法をかけている間に、他のセキュリティに関するコマンドを見ておきましょう。
   ...
   Options:
      -a, --add CERT                   信頼する証明書を追加します。      -l, --list                       信頼している証明書のリストを表示します。      -r, --remove STRING              STRING を含む証明書を削除します。      -b, --build EMAIL_ADDR           EMAIL_ADDR に対する秘密鍵や自己署名証明書を
                                       作成します。      -C, --certificate CERT           --sign で使用する証明書を指定します。      -K, --private-key KEY            --sign で使用する秘密鍵を指定します。      -s, --sign NEWCERT               自分の鍵を用いて証明書に署名します。  
