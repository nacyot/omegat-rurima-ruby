...
: install
: site-install
    作成した拡張ライブラリを $sitearchdir にインストールします。    カレントディレクトリにディレクトリ lib があれば
...
@param command コマンドを指定します。#@since 1.9.3
@param opts オプションを [[c:Hash]] で指定します。            :werror というキーに真を指定すると
#@end
...
@param files ファイルのリストか、[[m:Dir.glob]]で利用できる glob パターンを指定します。             最後の要素が [[c:Hash]] の場合は [[m:FileUtils.#rm_f]] の第二引数になります。
...
target が times の全ての要素よりも新しい場合は target の更新時刻を返します。그렇지 않으면 nil을 리턴합니다.target が存在しない場合も nil を返します。
...
@return 問題なくプリプロセスできたら true を返します。        プリプロセスに失敗したら false を返します。
...
@param pattern 「egrep の」正規表現を文字列で指定します。                Ruby の正規表現ではありません。
...
与えられた macro が共通のヘッダファイルか headers に定義されている場合は真を返します。그렇지 않으면 false를 리턴합니다.
...
@param func 検査する関数名を指定します。            nil または空文字列のときは、"main" になります。
...
@param func 関数名を指定します。            nil または空文字列を指定した場合は "main" になります。

@param paths ライブラリを検索するパスを文字列の配列で指定します。

--- find_header(header, *paths) -> bool

与えられた paths から header を検索し、見つかった場合は真を返します。그렇지 않으면 false를 리턴합니다.
...
#@since 1.9.2
@param preheaders ヘッダファイルを検査する前に読み込んでおくヘッダファイルを指定します。#@end
...
#@since 1.9.1
である場合、HAVE_STRUCT_FOO_BAR というプリプロセッサマクロをコンパイラに渡します。また、後方互換性のために HAVE_ST_BAR というプリプロセッサマクロも定義します。#@else
である場合、HAVE_ST_BAR というプリプロセッサマクロをコンパイラに渡します。#@end
...
@return オプションが指定されてた場合は true を、指定されなかった場合は
        ...
        (両方指定した場合はブロックが優先されます)。        オプションに引数が指定されていた場合は指定した文字列を返します。
...
예
  # xml2 の configure オプションを指定できるようにします。  xml2_dirs = dir_config('xml2', '/opt/local/include/libxml2', '/opt/local/lib')
...
@param target ターゲットとなる拡張ライブラリの名前を指定します。              例えば、拡張ライブラリで "Init_foo" という関数を定義して
              いる場合は、"foo" を指定します。              '/' を含む場合は、最後のスラッシュ以降のみをターゲット名
...
@param srcprefix ソースコードがあるディレクトリ名を指定します。                 省略した場合は extconf.rb があるディレクトリを使用します。                 
以下のようなディレクトリ構成の場合:
...
@param path パスを指定します。デフォルトは環境変数 PATH です。            環境変数 PATH が定義されていない場合は /usr/local/bin,
...
@return 見つかった依存関係を Makefile 形式で返します。#@end
...
以下の全ての検査に成功した場合は、真を返します。그렇지 않으면 false를 리턴합니다.
...
@param opt リンカに渡すオプションを指定します。           $CFLAGS, $LDFLAGS もリンカには渡されます。

@return 実行ファイルが存在する場合は true を返します。そうでない場合は
        false를 리턴합니다.プリプロセス、コンパイル、リンクのいずれかの
...
@param opt コンパイラに渡すオプションを指定します。           $CFLAGS もコンパイラには渡されます。
...
@return 定数 const がシステムに存在する場合はその値を返します。        定数 const がシステムに存在しない場合は nil を返します。
...
--- CONFIG -> Hash
#@since 1.8.5
[[m:RbConfig::MAKEFILE_CONFIG]] と同じです。#@else
[[m:Config::MAKEFILE_CONFIG]] と同じです。#@end
