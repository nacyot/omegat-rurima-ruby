...
#@else
UNIX シェルのコマンドライン解析に似た空白区切りの単語分割を行うモジュールです。#@end
...
@param line 分割の対象となる文字列を指定します。@return 分割結果の各文字列を要素とする配列を返します。#@since 1.8.7
@raise ArgumentError 引数の中に対でないシングルクォートまたはダブル
       クォートが現れた場合に発生します。#@else
@raise ArgumentError 引数が文字列でない場合か、引数の中に対でない
       シングルクォートまたはダブルクォートが現れた場合に発生します。#@end
...
@param array エスケープ対象の文字列を要素とする配列を指定します。@return エスケープ結果を連結した文字列を返します。#@end
...
@return 分割結果の各文字列を要素とする配列を返します。@raise ArgumentError 引数の中に対でないシングルクォートまたはダブル
       クォートが現れた場合に発生します。@see [[m:Shellwords.#shellsplit]]
...
@return エスケープされた文字列を返します。@see [[m:Shellwords.#shellescape]]
...
@return エスケープ結果を連結した文字列を返します。@see [[m:Shellwords.#shelljoin]]
#@end
