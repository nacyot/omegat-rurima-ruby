...
ARGV.getopts を使ってください。[[m:Object::ARGV]], [[m:OptionParser::Arguable#getopts]] を参照。#@end
...
解析結果は全て "$OPT_指定した引数名" という形で処理されます。
  ...
  * シングルオプションや引数を伴わないオプションが使用された場合は、
    `true' がセットされます。//emlist{
      -f        → $OPT_f = true  
      --version → $OPT_version = true
//}
  * その他はそのオプションの引数がセットされます。//emlist{
...
@param long_opts  ロングネームのオプションや、引数を伴うオプションを文字列で指定をします。                  --version や --geometry 300x400、-d host:0.0 等がこれに該当し
                  합니다.引数を伴う指定は ":" を必ず付ける様にします。この例の場合、
