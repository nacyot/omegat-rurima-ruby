...
context には hash table で要素のコンテキストを指定します。基本的には text node ([[c:REXML::Text]]) での特別な文字、特に空白について
の取り扱いを指定できます。#@include(context)
...
の取り扱いについての設定です。#@include(context)
...
これは、
  ...
  * context の :respect_whitespace に self の要素名が含まれていて、
    :compress_whitespace に self の要素名が含まれていない。    「含まれている」というのには :all が指定されている場合と、
    配列に含まれている場合の両方があります。のいずれかの場合に真を返します。
...
以下のいずれかであれば、raw モードであると判定されます。  * [[m:REXML::Element#context]] の :raw が :all である
...
--- size -> Integer
保持している要素の個数を返します。  require 'rexml/document'
