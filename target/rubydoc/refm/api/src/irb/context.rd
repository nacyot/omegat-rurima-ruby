...
: IRB.conf[:IRB_RC]
  ...
  ジェクトを実行します。ブロック引数には [[c:IRB::Context]] が渡されます。  そのため、サブ irb の設定をまとめて実行するのに使用します。
...
#@# になった際にコメントインします。#@#: IRB.conf[:RC_NAME_GENERATOR]
...
@return 出力結果に to_s したものを表示する場合は false を返します。それ
        以外の場合は true を返します。#@else
@return 出力結果に inspect したものを表示する場合は true を返します。        to_s したものを表示する場合は false を返します。#@end
...
@param opt 以下のいずれかを指定します。#@since 1.9.2
: false, :to_s, :raw
  出力結果を to_s したものを表示します。: true, :p, :inspect
  出力結果を inspect したものを表示します。: :pp, :pretty_inspect
  出力結果を pretty_inspect したものを表示します。: :yaml, :YAML
  出力結果を YAML 形式にしたものを表示します。: :marshal, :Marshal, :MARSHAL, [[c:Marshal]]
  出力結果を [[m:Marshal.#dump]] したものを表示します。

@see [[ref:lib:irb#inspect_mode]]
#@else
: true, nil
  出力結果を inspect したものを表示します。: false
  出力結果を to_s したものを表示します。#@end
...
#@# 変更しても影響がないため省略しました。#@#--- load_modules=
...
#@# 変更しても影響がないため省略しました。#@#--- use_readline=(opt)
...
@return 詳細なメッセージを出力するように設定されている場合は true を返
        합니다.そうでない場合は false か nil を返します。
...
と常に nil を返す不具合がある事に注意してください。#@end
...
#@# 使用されていないため省略しました。#@# --- workspace_home -> nil
