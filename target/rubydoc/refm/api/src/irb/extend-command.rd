...
@param cmd_name メソッド名を [[c:Symbol]] か文字列で指定します。                cmd_class で指定するクラスの execute メソッドとして定
                義してある必要があります。

@param cmd_class 指定した拡張が定義されたクラス名を [[c:Symbol]]、
                 [[c:String]]、[[c:Class]] のいずれかで指定します。                 なお、このクラスは [[c:IRB::ExtendCommand]] 以下で定義
                 する必要があります。
...
メソッドのエイリアスを定義します。라이브러리 내부에서 사용됩니다.
...
@param cmd_name メソッド名を [[c:Symbol]] で指定します。                [[c:IRB::Context]] クラスのインスタンスメソッドとして定
                義してある必要があります。
...
#@# --- CE
#@#
#@# 使用されていないようなので省略しました。#@# = module IRB::MethodExtender
#@#
#@# == Instance Methods
#@#
