...
def_e2message メソッドや def_exception メソッドが使えます。これらで例外クラスとメッセージを関連づけることができます。
...
@param message_format メッセージのフォーマットを指定します。                    [[m:Kernel.#sprintf]] のフォーマット文字列と同じ形式を使用できます。
...
@param superklass 定義する例外クラスのスーパークラスを指定します。                  省略すると [[c:StandardError]] を使用します。
...
@param superclass 定義する例外のスーパークラスを指定します。                  省略すると [[c:StandardError]] を使用します。
