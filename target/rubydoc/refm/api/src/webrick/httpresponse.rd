...
@param config 설정을 담고 있는 해쉬를 지정합니다.:HTTPVersion は必須です。
...
@param val メッセージボディを文字列か [[c:IO]] オブジェクトで指定します。           自身が chunked であってもチャンク形式にする必要はありません。           適切にチャンク形式エンコーディングされます。
...
Content-Length ヘッダの値を整数で表すアクセサです。기본 설정값은 nil입니다.
...
自身の内容があるファイルのそれである場合に、そのファイル名を返します。기본 설정값은 nil입니다.

--- filename=(file)

自身の内容があるファイルのそれである場合に、そのファイル名をセットします。기본 설정값은 nil입니다.
...
レスポンスの keep_alive が有効である場合は、真を返します。그렇지 않으면 false를 리턴합니다.
デフォルトは真です。
...
HTTP のレスポンスの最初の行の reason phrase を返します。この値が nil の場合 reason phrase は status から生成されます。기본 설정값은 nil입니다.

--- reason_phrase=(val)

HTTP のレスポンスの最初の行の reason phrase をセットします。この値が nil の場合 reason phrase は status から生成されます。기본 설정값은 nil입니다.
...
リクエストの HTTP メソッドをセットします。기본 설정값은 nil입니다.
...
リクエストの URI をセットします。기본 설정값은 nil입니다.
