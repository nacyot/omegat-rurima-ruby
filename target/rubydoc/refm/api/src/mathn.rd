...
require cmath
require prime
[[c:Rational]] と [[c:Complex]] をよりシームレスに利用できるようにするライブラリです。数値ライブラリの挙動をグローバルに変更します。#@else
require complex
require rational
[[lib:rational]] と [[lib:complex]] をシームレスに利用できるようにするライブラリです。数値ライブラリの挙動をグローバルに変更します。#@end

 * 整数の除算が割り切れない場合、 [[c:Rational]] オブジェクトを返すようになります。 * 複素数や有理数の演算結果が実数や整数に収まる場合、 [[c:Float]] オブジェクトや [[c:Integer]] オブジェクトを返します。 * [[c:Math]] モジュールの数学関数の定義域と終域を、実数のみから複素数へと拡大します。
...
現在のバージョンでは "3/5", "-17/7" のように10進数の既約分数表記を返します。#@end
= redefine Math
