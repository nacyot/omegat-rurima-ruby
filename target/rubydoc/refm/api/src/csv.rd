...
#@since 1.9.1
CSV (Comma Separated Values)를 다루는 라이브러리입니다.

#@# 설명을 해야함.
#@# 단순 번역이 아닌 내용을 작성

이 CSV 라이브러리는 FasterCSV에서 시작되었습니다.
FasterCSV는 Ruby 1.8 내장된 CSV 라이브러리를 대신하기 위해서 만들어졌습니다.
이 라이브러리는 사용자들의 문제를 해결하기 위해 디자인되었습니다. 주된 목표는 아래 3가지 입니다.

 (1) 순수 Ruby를 사용해 기존 CSV 라이브러리보다 훨씬 빠르게 만듦.
 (2) 최대한 적고 관리가 쉬운 코드 기반을 만듦.(FasterCSV는 구모도 크고,
     많은 기능을 포함하고 있습니다.구문 분석에 해당하는 코드는 상당히 적은 상태입니다.)
 (3) CSV 라이브러리의 인터페이스를 개선함.

마지막 목표는 주관적일 수 있는 문제입니다.변경해야만 하는 이유가 없는 한 원래의 인터페이스를 따르고 있기 때문에, 과거 CSV 라이브러리와 큰 차이는 없습니다.
 ...
 * パフォーマンスのため、インスタンスメソッドでセパレータを上書き出来なくなりました。   [[m:CSV.new]] でセットするようにしてください。
...
: :integer
  [[m:Kernel.#Integer]] を使用してフィールドを変換します。: :float
  [[m:Kernel.#Float]] を使用してフィールドを変換します。: :numeric
  :integer と :float の組み合わせです。: :date
  [[m:Date.parse]] を使用してフィールドを変換します。: :date_time
  [[m:DateTime.parse]] を使用してフィールドを変換します。: :all
...
: :downcase
  ヘッダの文字列に対して [[m:String#downcase]] を呼び出します。: :symbol
...
@param data [[c:String]] か [[c:IO]] のインスタンスを指定します。            [[c:String]] のインスタンスを指定した場合、[[m:CSV#string]] を使用して
            後からデータを取り出すことが出来ます。

@param options CSV をパースするためのオプションをハッシュで指定します。               パフォーマンス上の理由でインスタンスメソッドではオプションを上書きすることが
               出来ないので、上書きしたい場合は必ずここで上書きするようにしてください。

: :col_sep
  フィールドの区切り文字列を指定します。この文字列はパースする前にデータの
  エンコーディングに変換されます。: :row_sep
  行区切りの文字列を指定します。:auto という特別な値をセットすることができます。  :auto を指定した場合データから自動的に行区切りの文字列を見つけ出します。このとき
  データの先頭から次の "\r\n", "\n", "\r" の並びまでを読みます。  A sequence will be selected even if it occurs in a quoted field, assuming that you
...
: :quote_char
  フィールドをクオートする文字を指定します。長さ 1 の文字列でなければなりません。  正しいダブルクオートではなく間違ったシングルクオートを使用しているアプリケーション
  で便利です。  CSV will always consider a double  sequence this character to be an
  escaped quote.
  この文字列はパースする前にデータのエンコーディングに変換されます。: :field_size_limit
...
: :converters
  ...
  の場合は配列に格納する必要はありません。  全ての組み込みの変換器は、値を変換する前に UTF-8 にエンコーディング変
  換を試みます。エンコーディング変換に失敗した場合はフィールドは変換さ
  れません。: :unconverted_fields
  真をセットすると [[m:CSV::Row#unconverted_fields]] という変換前のフィー
  ルドを返すメソッドを全ての行に追加します。headers オプションによって
  追加したヘッダはフィールドではないので
  [[m:CSV::Row#unconverted_fields]] は空の配列を返します。: :headers
  :first_row というシンボルか真を指定すると、CSV ファイルの一行目をヘッダとして扱います。  配列を指定するとそれをヘッダとして扱います。文字列を指定すると [[m:CSV.parse_line]] を
  使用してパースした結果をヘッダとして扱います。このとき、:col_sep, :row_sep, :quote_char
  はこのインスタンスと同じものを使用します。この設定は [[m:CSV#shift]]
  の返り値を配列のかわりに [[c:CSV::Row]] のインスタンスに変更します。  [[m:CSV#read]] の返り値を配列の配列のかわりに [[c:CSV::Table]] のイン
  スタンスに変更します。: :return_headers
  偽を指定すると、ヘッダ行を無視します。真を指定すると、ヘッダ行を
  ヘッダと値が同一の [[c:CSV::Row]] のインスタンスとして返します。: :write_headers
  真を指定して :headers にも値をセットすると、ヘッダを出力します。: :header_converters
  :converters オプションに似ていますが、ヘッダ専用の変換器を定義します。  全ての組み込みの変換器は、値を変換する前に UTF-8 にエンコーディング変
  換を試みます。エンコーディング変換に失敗した場合はヘッダは変換されま
  せん。: :skip_blanks
  真を指定すると、空行を読み飛ばします。: :force_quotes
...
@param input [[c:String]] か [[c:IO]] のインスタンスを指定します。             デフォルトは [[c:ARGF]] です。

@param output [[c:String]] か [[c:IO]] のインスタンスを指定します。              デフォルトは [[m:$stdout]] です。

@param options ":in_", ":input_" で始まるキーは input にだけ適用されます。               ":out_", ":output_" で始まるキーは output にだけ適用されます。               それ以外のキーは両方に適用されます。               ":output_row_sep" のデフォルト値は [[m:$/]] です。
...
@param options [[m:CSV.new]] のオプションと同じオプションを指定できます。               :encoding というキーを使用すると入出力のエンコーディングを指定することができます。               [[m:Encoding.default_external]] と異なるエンコーディングを持つ入力を使用する場合は、
               必ずエンコーディングを指定してください。
...
@param options [[m:CSV.new]] のオプションと同じオプションを指定できます。               :encoding というキーを使用すると出力のエンコーディングを指定することができます。               ASCII と互換性の無い文字エンコーディングを持つ文字列を出力する場合は、このヒントを
               指定する必要があります。
...
@param options [[m:CSV.new]] のオプションと同じオプションを指定できます。               :encoding というキーを使用すると出力のエンコーディングを指定することができます。               :row_sep というキーの値には [[m:$/]] がセットされます。
...
@param options [[m:CSV.new]] のオプションと同じオプションを指定できます。               :encoding というキーを使用すると入力のエンコーディングを指定することができます。               入力のエンコーディングか [[m:Encoding.default_external]] と異なる場合は
               必ず指定しなければなりません。
...
@param row 配列か [[c:CSV::Row]] のインスタンスを指定します。           [[c:CSV::Row]] のインスタンスが指定された場合は、[[m:CSV::Row#fields]] の値
           のみが追加されます。
...
次に読み込まれる行が、ヘッダである場合に真を返します。그렇지 않은 경우엔 false를 반환합니다.
...
ヘッダを返す場合は、真を返します。그렇지 않은 경우엔 false를 반환합니다.
...
@return ヘッダを使用しない場合は配列を返します。        ヘッダを使用する場合は [[c:CSV::Row]] を返します。
...
パースした結果が unconverted_fields というメソッドを持つ場合に真を返します。그렇지 않은 경우엔 false를 반환합니다.
...
ヘッダを出力先に書き込む場合は真を返します。그렇지 않으면 false를 리턴합니다.
...
#@else
CSV (Comma Separated Values)를 라누느 라이브러리입니다.

= class CSV < Object

CSV (Comma Separated Values)를 다루는 클래스입니다.

各メソッドの共通パラメタ

  mode
     'r', 'w', 'rb', 'wb'중 하나를 지정할 수 있습니다.

     - 'r' 읽기
     - 'w' 쓰기
     - 'b' 바이너리 모드
  fs
     필드 구별 문자(혹은 문자열)     数値(Integer#chr で変換可能な値のみ)を指定した場合は対応する文字が区切り文字になります。     기본 설정값은 ','.
  rs
     행 구별 문자nil 지정시 기본값은 CrLf / Lf.
...
@param path パースするファイルのファイル名
@param rs 行区切り文字の指定。nil 지정시 기본값은 CrLf / Lf.
          Cr으로 나누고 싶으면 ?\r을 지정합니다.
...
@param src パースする配列
@param cells パースするフィールド数。@param out_dev csv形式の文字列の出力先。@param fs フィールドセパレータの指定。          기본설정은 nil이며, 이 때 구분자는 ','가 됩니다.
@param rs 행 구분 문자를 지정합니다.nil 지정시 기본값은 CrLf / Lf.
          Cr으로 나누고 싶으면 ?\r을 지정합니다.
...
@param index パース開始位置
@param out_dev 変換したデータの出力先。@param fs フィールドセパレータの指定。          기본설정은 nil이며, 이 때 구분자는 ','가 됩니다.
@param rs 행 구분 문자를 지정합니다.nil 지정시 기본값은 CrLf / Lf.
          Cr으로 나누고 싶으면 ?\r을 지정합니다.
