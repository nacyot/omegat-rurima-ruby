...
#@if (version <= "1.8.0")
@param dup dup が true の時は文字列を複製して freeze します。           dup が false なら複製せずに freeze します。#@else
@param dup dup は単に無視します。           引数の文字列は複製も freeze もされず、そのまま使います。#@end
...
文字列にはクラス名の他、以下の情報が含まれます。
    ...
    * スキャナポインタの現在位置。    * スキャン対象の文字列の長さ。    * スキャンポインタの前後にある文字。上記実行例の @ がスキャンポインタを表します。
...
#@else
  ...
  require 'strscan'
  s = StringScanner.new("るびい") # 문자코드는 UTF-8입니다.
  puts s.string      #=> るびい
  puts s.match?(/る/)  #=> 3
...
@param bytes 0 以上の整数を指定します。             ただし、スキャン対象の文字列の長さを超える分は無視されます。             bytes が 0 のとき、またはスキャンポインタが文字列の末尾を
             指しているときは空文字列 ("") を返します。
...
@param n 整数で、バイト単位で指定します。         負数を指定すると文字列の末尾からのオフセットとして扱います。@raise RangeError  マッチ対象の文字列の長さを超える値を指定すると発生します。
...
マッチに成功すると、s と f の値によって以下のように動作します。
    ...
    * s が true ならばスキャンポインタを進めます。    * s が false ならばスキャンポインタを進めません。    * f が true ならばマッチした部分文字列を返します。    * f が false ならばマッチした部分文字列の長さを返します。
...
他のメソッドと同等の動作になります。
    ...
    * scan_full(regexp, true, true) は [[m:StringScanner#scan]] と同等。    * scan_full(regexp, true, false) は [[m:StringScanner#skip]] と同等。    * scan_full(regexp, false, true) は [[m:StringScanner#check]] と同等。    * scan_full(regexp, false, false) は [[m:StringScanner#match?]] と同等。

@param regexp マッチに用いる正規表現を指定します。

@param s true ならばスキャンポインタを進めます。         false ならばスキャンポインタを進めません。

@param f true ならばマッチした部分文字列を返します。         false ならばマッチした部分文字列の長さを返します。
...
マッチに成功すると、s と f の値によって以下のように動作します。
    ...
    * s が true ならばスキャンポインタを進めます。    * s が false ならばスキャンポインタを進めません。    * f が true ならばスキャン開始位置からマッチした部分の末尾までの部分文字列を返します。    * f が false ならばスキャン開始位置からマッチした部分の末尾までの部分文字列の長さを返します。
...
他のメソッドと同等の動作になります。
    ...
    * search_full(regexp, true, true) は [[m:StringScanner#scan_until]] と同等。    * search_full(regexp, true, false) は [[m:StringScanner#skip_until]] と同等。    * search_full(regexp, false, true) は [[m:StringScanner#check_until]] と同等。    * search_full(regexp, false, false) は [[m:StringScanner#exist?]] と同等。
...
この仕様に依存したコードを書かないようにしましょう。
      ...
      s.string == str    # => true
      s.string.eql?(str) # => true (이후 false로 바뀔 가능성이 있습니다.)
...
@raise ScanError 2 回分以上戻そうとした時や、
                 まだマッチを一度も行っていない時、
                 前回のマッチが失敗していた時に発生します。#@end
예
      s = StringScanner.new('test string')
      begin
        # マッチを一度も行っていないので、例外が発生する。        s.unscan
...
#@end
      ...
      p s.scan(/\w+/) # => nil
      begin
        # 前回のマッチが失敗しているので、例外が発生する。        s.unscan
      rescue => err
        puts err
        # 출력 예
        #=> unscan failed: previous match had failed
