...
#@# 確かに require されているが、ファイルが存在しないためコメントアウト。#@# require rubygems/defaults/operating_system
...
=== gem コマンドの使い方
  ...
  $ gem help
    ...
    RubyGems は Ruby のための高機能なパッケージ管理ツールです。    これはより多くの情報へのポインタを含んでいる基本的なヘルプメッセージです。  
      使用方法:
        gem -h/--help
        gem -v/--version
...
: name
  この Gem の名前を指定します。: version
  この Gem のバージョンを指定します。: summary
  この Gem の短い説明を指定します。: files
  この Gem に含むファイルのリストを指定します。: authors
  この Gem の作者のリストを指定します。: email
  この Gem の作者の連絡先メールアドレスを指定します。: homepage
  この Gem のウェブサイトの URI を指定します。: description
  この Gem の長い説明を指定します。: rubyforge_project
...
@raise Gem::LoadError 指定された Gem やその Gem が必要とする Gem が見つからなかった場合に発生します。                      ただし、環境変数 GEM_SKIP に指定されている Gem に関してはこの例外は発生しません。
...
Windows プラットフォームであれば真を返します。그렇지 않으면 false를 리턴합니다.
