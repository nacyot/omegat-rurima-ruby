...
には freeze 情報が引き継がれません。したがって、生成された集合に対する要素の変更はエラーになりません。#@end
...
#@if (version >= "1.9.1")
@param enum 集合要素を格納するオブジェクトを指定します。       enum には each メソッドが定義されている必要があります。@raise NoMethodError 引数 enum が与えられて、かつ enum に each メソッドが
       定義されていない場合に発生します。#@else
@param enum Enumerable オブジェクトを指定します。@raise ArgumentError Enumerable オブジェクトでない引数が与えられた場合に
       발생합니다.
#@end
...
freeze 情報のコピーは実質的な効果を持ちません。#@else
Ruby 1.9 の Set クラスでは、dup と clone に共通して、内部記憶として
用いるハッシュも含めて taint 情報をコピーします。ただし、clone では内部記憶の freeze 情報はコピーされません。このため、freeze された集合を clone した場合、生成された集合の要素は
変更可能である点に注意してください。#@end
...
@param enum 置き換え後の集合要素を格納するオブジェクトを指定します。@raise NoMethodError 引数 enum に each メソッドが定義されていない場合に
       발생합니다.
#@else
@param enum 置き換え後の集合要素を格納する Enumerable オブジェクトを
            指定します。@raise ArgumentError 引数が Enumerable オブジェクトでない場合に発生します。#@end
...
@param enum 追加対象の要素を格納したオブジェクトを指定します。@raise NoMethodError 引数 enum に each メソッドが定義されていない場合に
       발생합니다.
#@else
@param enum 追加対象の要素を格納した Enumerate オブジェクトを指定します。@raise ArgumentError 引数が Enumerable オブジェクトでない場合に発生します。#@end
...
@param enum 削除対象の要素を格納したオブジェクトを指定します。@raise NoMethodError 引数 enum に each メソッドが定義されていない場合に
       발생합니다.
#@else
@param enum 削除対象の要素を格納した Enumerate オブジェクトを指定します。@raise ArgumentError 引数が Enumerable オブジェクトでない場合に発生します。#@end
...
#@if (version >= "1.9.1")
@param enum each メソッドが定義されたオブジェクトを指定します。@raise NoMethodError 引数 enum に each メソッドが定義されていない場合に
       발생합니다.
#@else
@param enum Enumerable オブジェクトを指定します。@raise ArgumentError 引数が Enumerable オブジェクトでない場合に発生します。#@end
