...
=== サンプルコード
  ...
  # ruby 1.8 では警告がでますが、動作します。  require 'tracer'
...
#@if (version < "1.9.1")
ruby 1.8 ではブロックを与えると警告がでます。#@end

フィルタは複数追加でき、
そのうち一つでも偽を返すとトレースの出力は抑制されます。

@param proc トレース出力するかどうかを決定する手続きオブジェクトを指定します。            通常、true か falseを返す必要があります。
...
真ならば、ビルトインメソッドの呼び出しを表示します。기본 설정은 false입니다.
