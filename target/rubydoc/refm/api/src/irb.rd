...
require irb/locale
#@# IRB.conf[:VERSION] を設定していない場合のみ。require irb/version
...
#@end
  ...
  --noreadline      readlineライブラリを利用しない。デフォルトでは
                    inf-ruby-mode 以外で readline ライブラリを利用する。  --prompt prompt-mode
  --prompt-mode prompt-mode
                    プロンプトモードを切り替える。現在定義されているプ
                    ロンプトモードは、default/simple/xmp/inf-ruby。  --inf-ruby-mode   emacsのinf-ruby-mode 用のプロンプト表示を行なう。                    特に指定がない限り readline ライブラリは使わなくなる。  --sample-book-mode
  --simple-prompt
                    非常にシンプルなプロンプトを用いるモード。  --noprompt        プロンプトを表示しない。  --tracer          コマンド実行時にトレースする。  --back-trace-limit n
                    バックトレース表示をバックトレースの頭から n、
                    うしろから n だけ行なう。デフォルト値は 16。  --context-mode n  新しいワークスペースを作成した時に関連する Binding
                    オブジェクトの作成方法を 0 から 3 で設定する。                    (IRB::Context 참조)
...
irb コマンドは起動時にホームディレクトリの .irbrc というファイルを読み込みます。.irbrc는 Ruby 스크립트입니다.ホームディレクトリに .irbrc が存在しない場合は、
...
====[a:customize_prompt] 프롬프트 커스터마이즈
...
: %l
    文字列中のタイプを表す (", ', /, ], `]'は%wの中の時)
: %NNi
    インデントのレベルを、NN 桁に右詰めした文字列。    NN は省略可能。: %NNn
...
例えば以下の実行例を見てください。
  ...
  irb(main):004:0> x = "OK"          # 지역 변수 x를 정의
  => "OK"
  irb(main):005:0> x                 # x를 출력
  => "OK"
  irb(main):006:0> irb               # 서브 irb를 실행
  irb#1(main):001:0> x               # x를 표시
...
===[a:configure_sub_irb] 서브 irb의 설정
...
irb のいろいろな使用例を以下に示します。
  ...
  nil
  irb#3(#<Foo:0x4010af3c>):002:0> foo         # f.foo를 실행
  nil
  irb#3(#<Foo:0x4010af3c>):003:0> bar         # f.bar를 실행
  barnil
...
: irb_context
  ...
  irb の現在の設定です。[[c:IRB::Context]] 객체입니다.
...
: irb_change_binding(*obj)
  ...
  irb の self を obj に変更します。  obj が省略されたときは、
...
: irb_source(path)
  ...
  価されます。[[m:$"]] 은 갱신되지 않으며 몇번이고 다시 불러올 수 있습니다.
...
#@end
  ...
  Ruby の load の irb 版です。  ファイル path を Ruby スクリプトとみなし、
  現在の irb インタプリタ上で実行します。  ただし、prev に true を指定した場合は実行は内部的に生成される無名モジュー
...
また、help コマンドは 1.8 系では動作しないバグがあります。#@end

=== 시스템 변수

: _

  방금 실행한 식(expression)의 실행결과입니다.
...
: __
  ...
  実行結果の履歴です。  __[lineno] で、lineno 行で実行した結果を得られます。  lineno が負の時は、最新の結果から -lineno 行だけ前の
  結果を得られます。
...
[[m:IRB::INSPECTORS.def_inspector]] を参照してください。#@end
...
irb를 종료합니다.ret で指定したオブジェクトを返します。
...
@param exception 発生させる例外を指定します。지정하지 않으면
                 [[c:IRB::Abort]] が発生します。
